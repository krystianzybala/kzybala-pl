 ...
 *
 * <p>Same detection code as content/labs/cache-hierarchy's
 * {@code CacheTopology} (that lab's four working sets are a subset of
 * this one's five — l1/l2/llc/memory there map to l1/l2/llc/large here);
 * this lab additionally names a "2xllc" tier that deliberately sits just
 * past the LLC boundary, between "fits" and "clearly DRAM-resident".
 */
public final class CacheTopology {
     ...
     * it while exceeding the previous level by construction; "2xllc"
     * deliberately exceeds the LLC without being as extreme as "large";
     * "large" is 8x the LLC so no cache level can hold it.
     */
    public int workingSetElements(String name) {
        long bytes = switch (name) {
            ...
            case "2xllc" -> llcBytes * 2;
            case "large" -> llcBytes * 8;
    ...
    public static List<String> workingSetNames() {
        return List.of("l1", "l2", "llc", "2xllc", "large");
    }
}

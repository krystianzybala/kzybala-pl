...
fn bench_shared_pool(c: &mut Criterion) {
    c.bench_function("shared_pool_increment_4_threads", |b| {
        b.iter(run_shared_pool)
    });
}

fn bench_owned_partitions(c: &mut Criterion) {
    c.bench_function("owned_partition_increment_4_threads", |b| {
        b.iter(run_owned_partitions)
    });
}

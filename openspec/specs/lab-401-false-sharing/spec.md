# Lab 401: False Sharing Specification

## Purpose

TBD - created by archiving change plab-010-false-sharing-reference-lab. Update Purpose after archive.

## Requirements

### Requirement: Performance question and falsifiable hypothesis
The laboratory SHALL state the question “Why can independent counters destroy throughput when they occupy the same cache line?” and the hypothesis “Coherence traffic, not logical sharing, causes the collapse; padding or ownership partitioning restores scalability.” before presenting results.
...
The laboratory SHALL explain the relevant concurrency mechanism before exposing optimized source code.
...
The laboratory SHALL provide runnable Java implementations focused on volatile fields, VarHandles, @Contended where explicitly enabled, manual padding and per-thread ownership.
...
The laboratory SHALL provide runnable Rust implementations focused on atomics, repr(align), padded wrappers and per-thread shards.
...
The laboratory SHALL include the controlled variants: adjacent counters, manually padded counters, runtime-assisted padding, per-thread shard plus reduction.
...
The laboratory SHALL exercise these data profiles where applicable: two threads, physical-core sweep, SMT sibling placement, cross-socket placement where available.
...
The laboratory SHALL capture and correctly label: ops/s, ns/update, cache-to-cache transfers, LLC misses, CPU utilization.
...
The laboratory SHALL attach relevant evidence from the repository-supported subset of JMH group benchmarks, Criterion/custom threaded harness, perf c2c, perf stat, async-profiler.
...
The laboratory SHALL explicitly discuss these traps: using thread count without topology; padding local objects that are not adjacent; claiming volatile itself is the cause; using different memory-order guarantees.
...
The laboratory SHALL provide data-driven or deterministic visualizations covering: cache-line ownership animation, throughput scaling curve, perf c2c evidence panel.

...
- [ ] 7. Create the required visual explanations: topology map, thread/IRQ placement overlay, latency variance chart.
- [ ] 8. Add terminology, assumptions and a short pre-lab diagnostic question.
- [ ] 9. Document the known traps: hardcoding CPU ids; requiring root for default run; claiming results portable across topology; pinning JVM GC/compiler threads indiscriminately.
- [ ] 10. Add accessible textual/table fallbacks for every essential visualization.

## 3. Java Implementation

- [ ] 11. Implement the Java baseline and required variants with focus on thread affinity integration via an optional platform adapter, pre-touching, JVM processor counts and topology metadata.
...
- [ ] 16. Implement the Rust baseline and required variants with focus on core-affinity integration, first touch, allocator/thread placement and topology metadata.
...
- [ ] 21. Add parameter coverage for: SPSC handoff, memory scan, UDP ingest.
- [ ] 22. Execute or wire the required variants: unpinned baseline, pinned isolated core, SMT siblings, same NUMA node, remote NUMA node, IRQ interference profile where safely configurable.
...
- [ ] 26. Capture these metrics: context switches, CPU migrations, remote NUMA accesses, p99/p999, cache transfers, interrupt rate.
- [ ] 27. Capture representative evidence using supported tools from: lscpu/numactl metadata, perf stat, perf c2c, taskset/affinity tools, JFR.
...
- [ ] 40. Run `openspec validate plab-603-cpu-affinity-numa-irq --strict` and repository quality gates.

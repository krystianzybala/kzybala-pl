...
1. **Phenomenon under measurement** — How much variance comes from where threads, memory and interrupts run rather than from application code?
2. **Primary hypothesis** — Pinning and first-touch placement can reduce migration and remote-memory noise, but incorrect topology choices or shared IRQs can make results worse.
3. **Controlled variables** — the variant axis (unpinned baseline, pinned isolated core, SMT siblings, same NUMA node, remote NUMA node, IRQ interference profile where safely configurable) and the
   dataset/profile axis (SPSC handoff, memory scan, UDP ingest) are varied one at a time; toolchain,
...
8. **Benchmark scenarios** — unpinned baseline, pinned isolated core, SMT siblings, same NUMA node, remote NUMA node, IRQ interference profile where safely configurable × SPSC handoff, memory scan, UDP ingest, each scenario
   selected per process invocation (never mixed in one run), named and
   recorded in the run's provenance.
9. **Expected canonical metrics** — context switches, CPU migrations, remote NUMA accesses, p99/p999, cache transfers, interrupt rate; imported through the
   plab-003 canonical schema with units, uncertainty and provenance.
10. **Profiler evidence** — repository-supported subset of: lscpu/numactl metadata, perf stat, perf c2c, taskset/affinity tools, JFR;
...
- concluding anything from a run that exhibits: hardcoding CPU ids
- concluding anything from a run that exhibits: requiring root for default run
- concluding anything from a run that exhibits: claiming results portable across topology
- concluding anything from a run that exhibits: pinning JVM GC/compiler threads indiscriminately
...
15. **Completion and verification gates** — the tasks.md completion gate
    ...
    accepted by the batch preflight, `openspec validate plab-603-cpu-affinity-numa-irq --strict`,
    and evidence maturity claimed no higher than derived
    (`docs/evidence-maturity.md`). Learning outcomes: capture hardware topology; apply pinning safely; distinguish application gains from placement artifacts.

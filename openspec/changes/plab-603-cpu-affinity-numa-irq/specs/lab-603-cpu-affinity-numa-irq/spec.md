# Delta for lab-603-cpu-affinity-numa-irq

## ADDED Requirements

### Requirement: Performance question and falsifiable hypothesis
The laboratory SHALL state the question “How much variance comes from where threads, memory and interrupts run rather than from application code?” and the hypothesis “Pinning and first-touch placement can reduce migration and remote-memory noise, but incorrect topology choices or shared IRQs can make results worse.” before presenting results.
...
The laboratory SHALL provide runnable Java implementations focused on thread affinity integration via an optional platform adapter, pre-touching, JVM processor counts and topology metadata.
...
The laboratory SHALL provide runnable Rust implementations focused on core-affinity integration, first touch, allocator/thread placement and topology metadata.
...
The laboratory SHALL include the controlled variants: unpinned baseline, pinned isolated core, SMT siblings, same NUMA node, remote NUMA node, IRQ interference profile where safely configurable.
...
The laboratory SHALL exercise these data profiles where applicable: SPSC handoff, memory scan, UDP ingest.
...
The laboratory SHALL capture and correctly label: context switches, CPU migrations, remote NUMA accesses, p99/p999, cache transfers, interrupt rate.
...
The laboratory SHALL attach relevant evidence from the repository-supported subset of lscpu/numactl metadata, perf stat, perf c2c, taskset/affinity tools, JFR.
...
The laboratory SHALL explicitly discuss these traps: hardcoding CPU ids; requiring root for default run; claiming results portable across topology; pinning JVM GC/compiler threads indiscriminately.
...
The laboratory SHALL provide data-driven or deterministic visualizations covering: topology map, thread/IRQ placement overlay, latency variance chart.

# Delta for lab-105-aos-vs-soa

## ADDED Requirements

### Requirement: Performance question and falsifiable hypothesis
The laboratory SHALL state the question “How does record layout affect cache density, SIMD potential and update cost?” and the hypothesis “SoA improves read-mostly scans over selected fields, while AoS can remain superior for whole-record operations and simpler ownership.” before presenting results.
...
The laboratory SHALL provide runnable Java implementations focused on Records/classes, primitive arrays, MemorySegment layouts and Vector API eligibility.
...
The laboratory SHALL provide runnable Rust implementations focused on repr(C) structs, Vec<Struct>, parallel Vec fields and alignment.
...
The laboratory SHALL include the controlled variants: AoS heap objects, AoS packed/off-heap, SoA primitive vectors, hybrid hot/cold split.
...
The laboratory SHALL exercise these data profiles where applicable: market-like quotes, positions with hot and cold fields, spatial points.
...
The laboratory SHALL capture and correctly label: ns/record, bytes/record, cache misses, allocations, vectorization evidence.
...
The laboratory SHALL attach relevant evidence from the repository-supported subset of JMH, Criterion, JOL, FFM layout inspection, perf, perfasm, cargo asm.
...
The laboratory SHALL explicitly discuss these traps: changing semantics across layouts; forgetting padding; measuring construction instead of scan; assuming SoA always wins.
...
The laboratory SHALL provide data-driven or deterministic visualizations covering: record layout diagram, cache-line occupancy view, throughput by accessed-field ratio.

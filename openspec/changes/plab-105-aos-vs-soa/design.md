# Design: Array of Structures vs Structure of Arrays
...
2. **Visualization** provides a deterministic or data-driven model: record layout diagram, cache-line occupancy view, throughput by accessed-field ratio.
3. **Java track** builds the baseline and optimized variants with focus on Records/classes, primitive arrays, MemorySegment layouts and Vector API eligibility.
4. **Rust track** builds equivalent variants with focus on repr(C) structs, Vec<Struct>, parallel Vec fields and alignment.
...
- AoS heap objects
- AoS packed/off-heap
- SoA primitive vectors
- hybrid hot/cold split

### Datasets and Profiles

- market-like quotes
- positions with hot and cold fields
- spatial points
...
- Primary focus: Records/classes, primitive arrays, MemorySegment layouts and Vector API eligibility.
...
- Primary focus: repr(C) structs, Vec<Struct>, parallel Vec fields and alignment.
...
1. **Phenomenon under measurement** — How does record layout affect cache density, SIMD potential and update cost?
2. **Primary hypothesis** — SoA improves read-mostly scans over selected fields, while AoS can remain superior for whole-record operations and simpler ownership.
3. **Controlled variables** — the variant axis (AoS heap objects, AoS packed/off-heap, SoA primitive vectors, hybrid hot/cold split) and the
   dataset/profile axis (market-like quotes, positions with hot and cold fields, spatial points) are varied one at a time; toolchain,
...
8. **Benchmark scenarios** — AoS heap objects, AoS packed/off-heap, SoA primitive vectors, hybrid hot/cold split × market-like quotes, positions with hot and cold fields, spatial points, each scenario
   selected per process invocation (never mixed in one run), named and
   recorded in the run's provenance.
9. **Expected canonical metrics** — ns/record, bytes/record, cache misses, allocations, vectorization evidence; imported through the
   plab-003 canonical schema with units, uncertainty and provenance.
10. **Profiler evidence** — repository-supported subset of: JMH, Criterion, JOL, FFM layout inspection, perf, perfasm, cargo asm;
...
- concluding anything from a run that exhibits: changing semantics across layouts
- concluding anything from a run that exhibits: forgetting padding
- concluding anything from a run that exhibits: measuring construction instead of scan
- concluding anything from a run that exhibits: assuming SoA always wins
...
15. **Completion and verification gates** — the tasks.md completion gate
    ...
    accepted by the batch preflight, `openspec validate plab-105-aos-vs-soa --strict`,
    and evidence maturity claimed no higher than derived
    (`docs/evidence-maturity.md`). Learning outcomes: choose layout from access patterns; quantify padding and density; explain hybrid layouts.

# Design: Allocation Cost and Object Layout

## Context

This is a new lab; use the shared framework and result pipeline rather than creating a custom mini-site.

This laboratory belongs to **Memory & Data**, level **Intermediate**. Its purpose is not to manufacture a dramatic Java-versus-Rust chart. Its purpose is to expose the mechanism, build equivalent implementations, measure them under controlled parameters and teach the learner how to verify or reject the hypothesis.
...
2. **Visualization** provides a deterministic or data-driven model: byte layout diagram, logical vs physical size, allocation flame graph.
3. **Java track** builds the baseline and optimized variants with focus on JOL, compressed references, records/classes, arrays, TLAB behavior and allocation profiling.
4. **Rust track** builds equivalent variants with focus on size_of, repr variants, Box/Vec, enum niche optimization and allocator behavior.
...
- packed off-heap struct
- reused mutable holder
...
- bytes/logical-record
- B/op
- allocation rate
- cache misses
- construction ns/record
...
- Primary focus: JOL, compressed references, records/classes, arrays, TLAB behavior and allocation profiling.
...
- Primary focus: size_of, repr variants, Box/Vec, enum niche optimization and allocator behavior.
...
1. **Phenomenon under measurement** — What are the real byte and CPU costs of convenient object models?
2. **Primary hypothesis** — Headers, alignment, references and allocator behavior can make logical data several times larger than packed representations.
3. **Controlled variables** — the variant axis (boxed object graph, flat primitive arrays, packed off-heap struct, reused mutable holder) and the
   dataset/profile axis (orders/quotes, tree nodes, small tuples) are varied one at a time; toolchain,
...
8. **Benchmark scenarios** — boxed object graph, flat primitive arrays, packed off-heap struct, reused mutable holder × orders/quotes, tree nodes, small tuples, each scenario
   selected per process invocation (never mixed in one run), named and
   recorded in the run's provenance.
9. **Expected canonical metrics** — bytes/logical-record, B/op, allocation rate, cache misses, construction ns/record; imported through the
   plab-003 canonical schema with units, uncertainty and provenance.
10. **Profiler evidence** — repository-supported subset of: JOL, JMH, JFR, async-profiler alloc, Criterion, size_of, heap profiler;
...
- concluding anything from a run that exhibits: ignoring alignment
- concluding anything from a run that exhibits: using packed unaligned fields unsafely
- concluding anything from a run that exhibits: comparing object graph with reduced functionality
- concluding anything from a run that exhibits: measuring only construction
...
15. **Completion and verification gates** — the tasks.md completion gate
    ...
    accepted by the batch preflight, `openspec validate plab-301-allocation-object-layout --strict`,
    and evidence maturity claimed no higher than derived
    (`docs/evidence-maturity.md`). Learning outcomes: calculate physical footprint; connect footprint to locality; choose representations deliberately.

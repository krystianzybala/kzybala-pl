# Delta for lab-301-allocation-object-layout

## ADDED Requirements

### Requirement: Performance question and falsifiable hypothesis
The laboratory SHALL state the question “What are the real byte and CPU costs of convenient object models?” and the hypothesis “Headers, alignment, references and allocator behavior can make logical data several times larger than packed representations.” before presenting results.
...
The laboratory SHALL provide runnable Java implementations focused on JOL, compressed references, records/classes, arrays, TLAB behavior and allocation profiling.
...
The laboratory SHALL provide runnable Rust implementations focused on size_of, repr variants, Box/Vec, enum niche optimization and allocator behavior.
...
The laboratory SHALL include the controlled variants: boxed object graph, flat primitive arrays, packed off-heap struct, reused mutable holder.
...
The laboratory SHALL exercise these data profiles where applicable: orders/quotes, tree nodes, small tuples.
...
The laboratory SHALL capture and correctly label: bytes/logical-record, B/op, allocation rate, cache misses, construction ns/record.
...
The laboratory SHALL attach relevant evidence from the repository-supported subset of JOL, JMH, JFR, async-profiler alloc, Criterion, size_of, heap profiler.
...
The laboratory SHALL explicitly discuss these traps: ignoring alignment; using packed unaligned fields unsafely; comparing object graph with reduced functionality; measuring only construction.
...
The laboratory SHALL provide data-driven or deterministic visualizations covering: byte layout diagram, logical vs physical size, allocation flame graph.

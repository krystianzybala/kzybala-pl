# Design: Syscall Amortization and Scatter/Gather I/O
...
2. **Visualization** provides a deterministic or data-driven model: user/kernel transition diagram, batch composition timeline, latency vs syscalls chart.
3. **Java track** builds the baseline and optimized variants with focus on gathering writes, direct buffers, batching policy and partial-write handling.
4. **Rust track** builds equivalent variants with focus on write_vectored, reusable IoSlice arrays, batching and partial-write state machine.
...
- size/time bounded batch
- backpressured receiver
...
- Primary focus: gathering writes, direct buffers, batching policy and partial-write handling.
...
- Primary focus: write_vectored, reusable IoSlice arrays, batching and partial-write state machine.
...
1. **Phenomenon under measurement** — How much does per-message kernel crossing cost, and when do gathering writes or batching pay off?
2. **Primary hypothesis** — Reducing syscall count improves throughput until buffering delay, copy construction or socket backpressure dominates.
3. **Controlled variables** — the variant axis (single small write, coalesced buffer, scatter/gather write, size/time bounded batch, backpressured receiver) and the
   dataset/profile axis (header plus payload, small messages, mixed sizes) are varied one at a time; toolchain,
...
8. **Benchmark scenarios** — single small write, coalesced buffer, scatter/gather write, size/time bounded batch, backpressured receiver × header plus payload, small messages, mixed sizes, each scenario
   selected per process invocation (never mixed in one run), named and
   recorded in the run's provenance.
9. **Expected canonical metrics** — syscalls/message, ns/message, throughput, p99 queue delay, bytes copied, partial writes; imported through the
   plab-003 canonical schema with units, uncertainty and provenance.
10. **Profiler evidence** — repository-supported subset of: custom socket/file harness, strace summary, perf stat, JFR, Criterion support harness;
...
- concluding anything from a run that exhibits: ignoring partial writes
- concluding anything from a run that exhibits: counting application calls instead of syscalls
- concluding anything from a run that exhibits: letting Nagle/coalescing differ silently
- concluding anything from a run that exhibits: measuring only throughput
...
15. **Completion and verification gates** — the tasks.md completion gate
    ...
    accepted by the batch preflight, `openspec validate plab-505-syscall-amortization-scatter-gather --strict`,
    and evidence maturity claimed no higher than derived
    (`docs/evidence-maturity.md`). Learning outcomes: implement correct vectored I/O; account for buffering delay; measure actual syscall reduction.

# Delta for lab-505-syscall-amortization-scatter-gather

## ADDED Requirements

### Requirement: Performance question and falsifiable hypothesis
The laboratory SHALL state the question “How much does per-message kernel crossing cost, and when do gathering writes or batching pay off?” and the hypothesis “Reducing syscall count improves throughput until buffering delay, copy construction or socket backpressure dominates.” before presenting results.
...
The laboratory SHALL provide runnable Java implementations focused on gathering writes, direct buffers, batching policy and partial-write handling.
...
The laboratory SHALL provide runnable Rust implementations focused on write_vectored, reusable IoSlice arrays, batching and partial-write state machine.
...
The laboratory SHALL include the controlled variants: single small write, coalesced buffer, scatter/gather write, size/time bounded batch, backpressured receiver.
...
The laboratory SHALL exercise these data profiles where applicable: header plus payload, small messages, mixed sizes.
...
The laboratory SHALL capture and correctly label: syscalls/message, ns/message, throughput, p99 queue delay, bytes copied, partial writes.
...
The laboratory SHALL attach relevant evidence from the repository-supported subset of custom socket/file harness, strace summary, perf stat, JFR, Criterion support harness.
...
The laboratory SHALL explicitly discuss these traps: ignoring partial writes; counting application calls instead of syscalls; letting Nagle/coalescing differ silently; measuring only throughput.
...
The laboratory SHALL provide data-driven or deterministic visualizations covering: user/kernel transition diagram, batch composition timeline, latency vs syscalls chart.

...
- [x] 7. Create the required visual explanations: assumption lifecycle diagram, deopt spike timeline, profile-before/after comparison.
- [x] 8. Add terminology, assumptions and a short pre-lab diagnostic question.
- [x] 9. Document the known traps: using artificial class loading without disclosure; hiding warm-up phase transitions; equating Rust branch misprediction with JVM deoptimization; publishing only steady-state mean.
- [x] 10. Add accessible textual/table fallbacks for every essential visualization.

## 3. Java Implementation

- [x] 11. Implement the Java baseline and required variants with focus on Uncommon traps, class hierarchy changes, null/range speculation and deoptimization logs.
...
- [x] 16. Implement the Rust baseline and required variants with focus on Static optimization assumptions, cold paths, panic/unwind configuration and branch profile changes as contrast.
...
- [x] 21. Add parameter coverage for: strategy dispatch, parsing mixed records, rare validation failure.
- [x] 22. Execute or wire the required variants: stable type profile, profile shift after warm-up, rare exception path, late subtype loading, nullability shift.
...
- [x] 40. Run `openspec validate plab-204-deoptimization-uncommon-traps --strict` and repository quality gates.

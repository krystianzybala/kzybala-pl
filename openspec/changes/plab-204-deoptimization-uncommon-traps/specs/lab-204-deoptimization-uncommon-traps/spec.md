# Delta for lab-204-deoptimization-uncommon-traps

## ADDED Requirements

### Requirement: Performance question and falsifiable hypothesis
The laboratory SHALL state the question “What happens when optimized code assumptions stop being true in production?” and the hypothesis “Profile-guided speculation can make common paths fast but profile shifts, class loading and uncommon cases can trigger deoptimization and tail spikes.” before presenting results.
...
The laboratory SHALL provide runnable Java implementations focused on Uncommon traps, class hierarchy changes, null/range speculation and deoptimization logs.
...
The laboratory SHALL provide runnable Rust implementations focused on Static optimization assumptions, cold paths, panic/unwind configuration and branch profile changes as contrast.
...
The laboratory SHALL include the controlled variants: stable type profile, profile shift after warm-up, rare exception path, late subtype loading, nullability shift.
...
The laboratory SHALL exercise these data profiles where applicable: strategy dispatch, parsing mixed records, rare validation failure.
...
The laboratory SHALL capture and correctly label: p99/p999, deoptimization count, recompilation count, latency timeline, code-cache events.
...
The laboratory SHALL attach relevant evidence from the repository-supported subset of JMH custom iteration logging, JFR, JITWatch, HotSpot logs, Criterion.
...
The laboratory SHALL explicitly discuss these traps: using artificial class loading without disclosure; hiding warm-up phase transitions; equating Rust branch misprediction with JVM deoptimization; publishing only steady-state mean.
...
The laboratory SHALL provide data-driven or deterministic visualizations covering: assumption lifecycle diagram, deopt spike timeline, profile-before/after comparison.

...
1. **Phenomenon under measurement** — What happens when optimized code assumptions stop being true in production?
2. **Primary hypothesis** — Profile-guided speculation can make common paths fast but profile shifts, class loading and uncommon cases can trigger deoptimization and tail spikes.
3. **Controlled variables** — the variant axis (stable type profile, profile shift after warm-up, rare exception path, late subtype loading, nullability shift) and the
   dataset/profile axis (strategy dispatch, parsing mixed records, rare validation failure) are varied one at a time; toolchain,
...
8. **Benchmark scenarios** — stable type profile, profile shift after warm-up, rare exception path, late subtype loading, nullability shift × strategy dispatch, parsing mixed records, rare validation failure, each scenario
   selected per process invocation (never mixed in one run), named and
   recorded in the run's provenance.
9. **Expected canonical metrics** — p99/p999, deoptimization count, recompilation count, latency timeline, code-cache events; imported through the
   plab-003 canonical schema with units, uncertainty and provenance.
10. **Profiler evidence** — repository-supported subset of: JMH custom iteration logging, JFR, JITWatch, HotSpot logs, Criterion;
...
- concluding anything from a run that exhibits: using artificial class loading without disclosure
- concluding anything from a run that exhibits: hiding warm-up phase transitions
- concluding anything from a run that exhibits: equating Rust branch misprediction with JVM deoptimization
- concluding anything from a run that exhibits: publishing only steady-state mean
...
15. **Completion and verification gates** — the tasks.md completion gate
    ...
    accepted by the batch preflight, `openspec validate plab-204-deoptimization-uncommon-traps --strict`,
    and evidence maturity claimed no higher than derived
    (`docs/evidence-maturity.md`). Learning outcomes: diagnose deoptimization; design stable hot-path profiles; include phase changes in latency analysis.

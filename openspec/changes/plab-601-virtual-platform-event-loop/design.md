...
1. **Phenomenon under measurement** — Which concurrency model fits blocking I/O, CPU-bound work and strict tail-latency paths?
2. **Primary hypothesis** — Virtual threads improve blocking-task scalability but do not create CPU capacity or guarantee low-latency scheduling; event loops and pinned platform threads remain useful for owned hot paths.
3. **Controlled variables** — the variant axis (thread-per-request platform, virtual thread per task, fixed event loop, CPU-bound pool, mixed blocking and CPU workload) and the
   dataset/profile axis (simulated socket wait, short CPU stage, long CPU stage, lock/native pinning case) are varied one at a time; toolchain,
...
8. **Benchmark scenarios** — thread-per-request platform, virtual thread per task, fixed event loop, CPU-bound pool, mixed blocking and CPU workload × simulated socket wait, short CPU stage, long CPU stage, lock/native pinning case, each scenario
   selected per process invocation (never mixed in one run), named and
   recorded in the run's provenance.
9. **Expected canonical metrics** — throughput, p50/p99/p999, scheduler CPU, thread count, context switches, pinning events; imported through the
   plab-003 canonical schema with units, uncertainty and provenance.
10. **Profiler evidence** — repository-supported subset of: JFR, async-profiler wall/cpu, JMH or macro harness, Criterion/custom Rust harness, perf sched;
...
- concluding anything from a run that exhibits: using sleep as the only I/O model
- concluding anything from a run that exhibits: claiming virtual threads are low-latency threads
- concluding anything from a run that exhibits: adding Tokio only to win a chart
- concluding anything from a run that exhibits: mixing CPU capacity across models
...
15. **Completion and verification gates** — the tasks.md completion gate
    ...
    accepted by the batch preflight, `openspec validate plab-601-virtual-platform-event-loop --strict`,
    and evidence maturity claimed no higher than derived
    (`docs/evidence-maturity.md`). Learning outcomes: select model per workload; recognize virtual-thread pinning and CPU limits; keep hot paths isolated.

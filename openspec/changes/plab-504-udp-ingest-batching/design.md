...
1. **Phenomenon under measurement** — What throughput and tail-latency trade-offs emerge when ingesting datagrams directly into a bounded processing path?
2. **Primary hypothesis** — Batching amortizes syscalls but increases queueing latency; socket buffers, drops and CPU placement determine behavior under burst load.
3. **Controlled variables** — the variant axis (one packet per receive, reused direct buffer, batched Linux receive where supported, copying handoff, zero-copy view handoff with bounded lifetime) and the
   dataset/profile axis (64B, 256B, 1400B datagrams, steady rate, microburst, overload) are varied one at a time; toolchain,
...
8. **Benchmark scenarios** — one packet per receive, reused direct buffer, batched Linux receive where supported, copying handoff, zero-copy view handoff with bounded lifetime × 64B, 256B, 1400B datagrams, steady rate, microburst, overload, each scenario
   selected per process invocation (never mixed in one run), named and
   recorded in the run's provenance.
9. **Expected canonical metrics** — packets/s, drop rate, p50/p99/p999, syscalls/packet, CPU, queue depth; imported through the
   plab-003 canonical schema with units, uncertainty and provenance.
10. **Profiler evidence** — repository-supported subset of: custom load generator, perf stat, ss/netstat counters, JFR, HdrHistogram;
...
- concluding anything from a run that exhibits: testing only loopback without labeling
- concluding anything from a run that exhibits: ignoring kernel drops
- concluding anything from a run that exhibits: unbounded queues hiding loss
- concluding anything from a run that exhibits: reusing receive buffer after publication
...
15. **Completion and verification gates** — the tasks.md completion gate
    ...
    accepted by the batch preflight, `openspec validate plab-504-udp-ingest-batching --strict`,
    and evidence maturity claimed no higher than derived
    (`docs/evidence-maturity.md`). Learning outcomes: build bounded UDP ingest; measure drops explicitly; select batch size from latency budget.

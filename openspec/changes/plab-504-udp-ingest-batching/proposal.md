# Proposal: UDP Ingest, Batching and Packet Loss
...
- Include these controlled variants: one packet per receive, reused direct buffer, batched Linux receive where supported, copying handoff, zero-copy view handoff with bounded lifetime.
- Exercise these datasets/profiles: 64B, 256B, 1400B datagrams, steady rate, microburst, overload.
- Record at least these metrics: packets/s, drop rate, p50/p99/p999, syscalls/packet, CPU, queue depth.
- Capture evidence using repository-supported subsets of: custom load generator, perf stat, ss/netstat counters, JFR, HdrHistogram.

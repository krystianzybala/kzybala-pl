# Delta for lab-305-zero-copy-binary-parsing

## ADDED Requirements

### Requirement: Performance question and falsifiable hypothesis
The laboratory SHALL state the question “How can a decoder expose typed fields without copying payloads or allocating object graphs?” and the hypothesis “Views over validated immutable bytes can eliminate copies and allocations, but lifetime, endian, bounds and mutation rules must be explicit.” before presenting results.
...
The laboratory SHALL provide runnable Java implementations focused on MemorySegment slices, ValueLayout endianness, flyweight readers and validated frame boundaries.
...
The laboratory SHALL provide runnable Rust implementations focused on borrowed byte slices, nom/manual parsing, lifetime-bound views and bytemuck only where sound.
...
The laboratory SHALL include the controlled variants: copying decoder, object-building decoder, validated zero-copy view, lazy field decode, mutable in-place update controlled variant.
...
The laboratory SHALL exercise these data profiles where applicable: fixed header plus variable payload, nested repeated fields, UTF-8 field.
...
The laboratory SHALL capture and correctly label: ns/message, B/message, bytes copied, branch misses, validation cost.
...
The laboratory SHALL attach relevant evidence from the repository-supported subset of JMH, Criterion, JFR allocation profile, perf, perfasm, cargo asm, miri/fuzzing.
...
The laboratory SHALL explicitly discuss these traps: returning views past buffer lifetime; skipping validation; using invalid UTF-8 unchecked; claiming zero-copy while converting strings.
...
The laboratory SHALL provide data-driven or deterministic visualizations covering: frame/view lifetime diagram, copy counter visualization, decoder pipeline.

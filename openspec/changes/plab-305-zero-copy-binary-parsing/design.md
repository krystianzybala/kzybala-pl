# Design: Zero-Copy Binary Parsing and Views
...
2. **Visualization** provides a deterministic or data-driven model: frame/view lifetime diagram, copy counter visualization, decoder pipeline.
3. **Java track** builds the baseline and optimized variants with focus on MemorySegment slices, ValueLayout endianness, flyweight readers and validated frame boundaries.
4. **Rust track** builds equivalent variants with focus on borrowed byte slices, nom/manual parsing, lifetime-bound views and bytemuck only where sound.
...
- copying decoder
- object-building decoder
- validated zero-copy view
- lazy field decode
- mutable in-place update controlled variant
...
- Primary focus: MemorySegment slices, ValueLayout endianness, flyweight readers and validated frame boundaries.
...
- Primary focus: borrowed byte slices, nom/manual parsing, lifetime-bound views and bytemuck only where sound.
...
1. **Phenomenon under measurement** — How can a decoder expose typed fields without copying payloads or allocating object graphs?
2. **Primary hypothesis** — Views over validated immutable bytes can eliminate copies and allocations, but lifetime, endian, bounds and mutation rules must be explicit.
3. **Controlled variables** — the variant axis (copying decoder, object-building decoder, validated zero-copy view, lazy field decode, mutable in-place update controlled variant) and the
   dataset/profile axis (fixed header plus variable payload, nested repeated fields, UTF-8 field) are varied one at a time; toolchain,
...
8. **Benchmark scenarios** — copying decoder, object-building decoder, validated zero-copy view, lazy field decode, mutable in-place update controlled variant × fixed header plus variable payload, nested repeated fields, UTF-8 field, each scenario
   selected per process invocation (never mixed in one run), named and
   recorded in the run's provenance.
9. **Expected canonical metrics** — ns/message, B/message, bytes copied, branch misses, validation cost; imported through the
   plab-003 canonical schema with units, uncertainty and provenance.
10. **Profiler evidence** — repository-supported subset of: JMH, Criterion, JFR allocation profile, perf, perfasm, cargo asm, miri/fuzzing;
...
- concluding anything from a run that exhibits: returning views past buffer lifetime
- concluding anything from a run that exhibits: skipping validation
- concluding anything from a run that exhibits: using invalid UTF-8 unchecked
- concluding anything from a run that exhibits: claiming zero-copy while converting strings
...
15. **Completion and verification gates** — the tasks.md completion gate
    ...
    accepted by the batch preflight, `openspec validate plab-305-zero-copy-binary-parsing --strict`,
    and evidence maturity claimed no higher than derived
    (`docs/evidence-maturity.md`). Learning outcomes: implement lifetime-safe views; separate validation from access; measure actual bytes copied.

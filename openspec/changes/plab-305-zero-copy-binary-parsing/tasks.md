...
- [x] 7. Create the required visual explanations: frame/view lifetime diagram, copy counter visualization, decoder pipeline.
- [x] 8. Add terminology, assumptions and a short pre-lab diagnostic question.
- [x] 9. Document the known traps: returning views past buffer lifetime; skipping validation; using invalid UTF-8 unchecked; claiming zero-copy while converting strings.
- [x] 10. Add accessible textual/table fallbacks for every essential visualization.

## 3. Java Implementation

- [x] 11. Implement the Java baseline and required variants with focus on MemorySegment slices, ValueLayout endianness, flyweight readers and validated frame boundaries.
...
- [x] 16. Implement the Rust baseline and required variants with focus on borrowed byte slices, nom/manual parsing, lifetime-bound views and bytemuck only where sound.
...
- [x] 21. Add parameter coverage for: fixed header plus variable payload, nested repeated fields, UTF-8 field.
- [x] 22. Execute or wire the required variants: copying decoder, object-building decoder, validated zero-copy view, lazy field decode, mutable in-place update controlled variant.
...
- [ ] 26. Capture these metrics: ns/message, B/message, bytes copied, branch misses, validation cost.
- [ ] 27. Capture representative evidence using supported tools from: JMH, Criterion, JFR allocation profile, perf, perfasm, cargo asm, miri/fuzzing.
...
- [x] 40. Run `openspec validate plab-305-zero-copy-binary-parsing --strict` and repository quality gates.

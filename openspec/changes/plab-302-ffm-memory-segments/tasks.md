...
- [x] 7. Create the required visual explanations: heap/off-heap ownership diagram, access-path assembly, copy accounting view.
- [x] 8. Add terminology, assumptions and a short pre-lab diagnostic question.
- [x] 9. Document the known traps: off-heap as automatic speedup; closing arena during use; cross-thread confinement violations; unsafe Rust without invariant documentation.
- [x] 10. Add accessible textual/table fallbacks for every essential visualization.

## 3. Java Implementation

- [x] 11. Implement the Java baseline and required variants with focus on MemorySegment, ValueLayout, VarHandle access, arenas, slicing and confinement.
...
- [x] 16. Implement the Rust baseline and required variants with focus on Vec, Box<[T]>, raw pointers and mmap-backed slices as comparison.
...
- [x] 21. Add parameter coverage for: fixed records, large numeric buffers, binary frames.
- [x] 22. Execute or wire the required variants: heap primitive array, confined segment, shared segment, sliced view, copied boundary crossing.
...
- [x] 40. Run `openspec validate plab-302-ffm-memory-segments --strict` and repository quality gates.

# Delta for lab-302-ffm-memory-segments

## ADDED Requirements

### Requirement: Performance question and falsifiable hypothesis
The laboratory SHALL state the question “When does moving data off heap help, and what costs replace GC-managed objects?” and the hypothesis “FFM can provide explicit layout and lifetime control, but access checks, arenas, copying and ownership errors can erase gains.” before presenting results.
...
The laboratory SHALL provide runnable Java implementations focused on MemorySegment, ValueLayout, VarHandle access, arenas, slicing and confinement.
...
The laboratory SHALL provide runnable Rust implementations focused on Vec, Box<[T]>, raw pointers and mmap-backed slices as comparison.
...
The laboratory SHALL include the controlled variants: heap primitive array, confined segment, shared segment, sliced view, copied boundary crossing.
...
The laboratory SHALL exercise these data profiles where applicable: fixed records, large numeric buffers, binary frames.
...
The laboratory SHALL capture and correctly label: ns/access, ns/record, allocation rate, RSS, cache misses, copy bytes.
...
The laboratory SHALL attach relevant evidence from the repository-supported subset of JMH, JFR, perfasm, Criterion, perf, miri for safety checks where relevant.
...
The laboratory SHALL explicitly discuss these traps: off-heap as automatic speedup; closing arena during use; cross-thread confinement violations; unsafe Rust without invariant documentation.
...
The laboratory SHALL provide data-driven or deterministic visualizations covering: heap/off-heap ownership diagram, access-path assembly, copy accounting view.

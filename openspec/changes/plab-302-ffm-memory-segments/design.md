# Design: Heap vs Off-Heap with FFM MemorySegment
...
2. **Visualization** provides a deterministic or data-driven model: heap/off-heap ownership diagram, access-path assembly, copy accounting view.
3. **Java track** builds the baseline and optimized variants with focus on MemorySegment, ValueLayout, VarHandle access, arenas, slicing and confinement.
4. **Rust track** builds equivalent variants with focus on Vec, Box<[T]>, raw pointers and mmap-backed slices as comparison.
...
- Criterion
- perf
- miri for safety checks where relevant
...
- Primary focus: MemorySegment, ValueLayout, VarHandle access, arenas, slicing and confinement.
...
- Primary focus: Vec, Box<[T]>, raw pointers and mmap-backed slices as comparison.
...
1. **Phenomenon under measurement** — When does moving data off heap help, and what costs replace GC-managed objects?
2. **Primary hypothesis** — FFM can provide explicit layout and lifetime control, but access checks, arenas, copying and ownership errors can erase gains.
3. **Controlled variables** — the variant axis (heap primitive array, confined segment, shared segment, sliced view, copied boundary crossing) and the
   dataset/profile axis (fixed records, large numeric buffers, binary frames) are varied one at a time; toolchain,
...
8. **Benchmark scenarios** — heap primitive array, confined segment, shared segment, sliced view, copied boundary crossing × fixed records, large numeric buffers, binary frames, each scenario
   selected per process invocation (never mixed in one run), named and
   recorded in the run's provenance.
9. **Expected canonical metrics** — ns/access, ns/record, allocation rate, RSS, cache misses, copy bytes; imported through the
   plab-003 canonical schema with units, uncertainty and provenance.
10. **Profiler evidence** — repository-supported subset of: JMH, JFR, perfasm, Criterion, perf, miri for safety checks where relevant;
...
- concluding anything from a run that exhibits: off-heap as automatic speedup
- concluding anything from a run that exhibits: closing arena during use
- concluding anything from a run that exhibits: cross-thread confinement violations
- concluding anything from a run that exhibits: unsafe Rust without invariant documentation
...
15. **Completion and verification gates** — the tasks.md completion gate
    ...
    accepted by the batch preflight, `openspec validate plab-302-ffm-memory-segments --strict`,
    and evidence maturity claimed no higher than derived
    (`docs/evidence-maturity.md`). Learning outcomes: choose heap vs off-heap from lifecycle; use segments safely; account for boundary copies.

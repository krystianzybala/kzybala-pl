...
- [ ] 7. Create the required visual explanations: ring state animation, sequence timeline, latency/CPU wait-strategy chart.
- [ ] 8. Add terminology, assumptions and a short pre-lab diagnostic question.
- [ ] 9. Document the known traps: using SPSC with multiple producers; publishing before payload write; coordinated omission; ignoring shutdown and wraparound.
- [ ] 10. Add accessible textual/table fallbacks for every essential visualization.

## 3. Java Implementation

- [ ] 11. Implement the Java baseline and required variants with focus on VarHandles, padded sequences, preallocated slots, batching and wait strategies.
...
- [ ] 16. Implement the Rust baseline and required variants with focus on atomics, MaybeUninit or initialized slots with documented invariants, cache padding and wait strategies.
...
- [ ] 21. Add parameter coverage for: primitive messages, fixed binary frames, burst and steady traffic.
- [ ] 22. Execute or wire the required variants: blocking queue baseline, naive volatile ring, padded acquire/release ring, batched ring, busy-spin/yield/park wait strategies.

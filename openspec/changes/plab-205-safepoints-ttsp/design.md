# Design: Safepoints and Time to Safepoint
...
2. **Visualization** provides a deterministic or data-driven model: safepoint handshake timeline, TTSP decomposition chart, thread-state view.
3. **Java track** builds the baseline and optimized variants with focus on Safepoint logs/JFR events, polling behavior, long counted loops, JNI/FFM boundaries and thread states.
4. **Rust track** builds equivalent variants with focus on Stop-the-world coordination implemented explicitly as a contrast, signal/preemption limits and long-running loops.
...
- cooperative loop
- long loop with sparse polls
- thread in native call
- many idle threads
...
- Primary focus: Safepoint logs/JFR events, polling behavior, long counted loops, JNI/FFM boundaries and thread states.
...
- Primary focus: Stop-the-world coordination implemented explicitly as a contrast, signal/preemption limits and long-running loops.
...
1. **Phenomenon under measurement** — Why can a short stop-the-world operation still produce a long application pause?
2. **Primary hypothesis** — Pause time includes both safepoint operation duration and time for threads to reach a safepoint; long loops and native sections can dominate TTSP.
3. **Controlled variables** — the variant axis (cooperative loop, long loop with sparse polls, thread in native call, many idle threads, allocation pressure trigger) and the
   dataset/profile axis (numeric loop, native sleep/downcall, thread fleet) are varied one at a time; toolchain,
...
8. **Benchmark scenarios** — cooperative loop, long loop with sparse polls, thread in native call, many idle threads, allocation pressure trigger × numeric loop, native sleep/downcall, thread fleet, each scenario
   selected per process invocation (never mixed in one run), named and
   recorded in the run's provenance.
9. **Expected canonical metrics** — TTSP, safepoint operation time, application stopped time, p99 latency, thread count; imported through the
   plab-003 canonical schema with units, uncertainty and provenance.
10. **Profiler evidence** — repository-supported subset of: JFR, safepoint logging, async-profiler wall-clock, perf, custom Rust coordinator;
...
- concluding anything from a run that exhibits: blaming GC for all pauses
- concluding anything from a run that exhibits: using obsolete flags without version checks
- concluding anything from a run that exhibits: claiming Rust has no pauses
- concluding anything from a run that exhibits: running with uncontrolled OS scheduling
...
15. **Completion and verification gates** — the tasks.md completion gate
    ...
    accepted by the batch preflight, `openspec validate plab-205-safepoints-ttsp --strict`,
    and evidence maturity claimed no higher than derived
    (`docs/evidence-maturity.md`). Learning outcomes: separate TTSP from operation time; identify non-cooperative work; reason about runtime coordination costs.

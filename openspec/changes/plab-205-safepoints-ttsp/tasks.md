...
- [x] 7. Create the required visual explanations: safepoint handshake timeline, TTSP decomposition chart, thread-state view.
- [x] 8. Add terminology, assumptions and a short pre-lab diagnostic question.
- [x] 9. Document the known traps: blaming GC for all pauses; using obsolete flags without version checks; claiming Rust has no pauses; running with uncontrolled OS scheduling.
- [x] 10. Add accessible textual/table fallbacks for every essential visualization.

## 3. Java Implementation

- [x] 11. Implement the Java baseline and required variants with focus on Safepoint logs/JFR events, polling behavior, long counted loops, JNI/FFM boundaries and thread states.
...
- [x] 16. Implement the Rust baseline and required variants with focus on Stop-the-world coordination implemented explicitly as a contrast, signal/preemption limits and long-running loops.
...
- [x] 21. Add parameter coverage for: numeric loop, native sleep/downcall, thread fleet.
- [x] 22. Execute or wire the required variants: cooperative loop, long loop with sparse polls, thread in native call, many idle threads, allocation pressure trigger.
...
- [ ] 26. Capture these metrics: TTSP, safepoint operation time, application stopped time, p99 latency, thread count.
- [ ] 27. Capture representative evidence using supported tools from: JFR, safepoint logging, async-profiler wall-clock, perf, custom Rust coordinator.
...
- [x] 40. Run `openspec validate plab-205-safepoints-ttsp --strict` and repository quality gates.

...
- [ ] 7. Create the required visual explanations: safepoint handshake timeline, TTSP decomposition chart, thread-state view.
- [ ] 8. Add terminology, assumptions and a short pre-lab diagnostic question.
- [ ] 9. Document the known traps: blaming GC for all pauses; using obsolete flags without version checks; claiming Rust has no pauses; running with uncontrolled OS scheduling.
- [ ] 10. Add accessible textual/table fallbacks for every essential visualization.

## 3. Java Implementation

- [ ] 11. Implement the Java baseline and required variants with focus on Safepoint logs/JFR events, polling behavior, long counted loops, JNI/FFM boundaries and thread states.
...
- [ ] 16. Implement the Rust baseline and required variants with focus on Stop-the-world coordination implemented explicitly as a contrast, signal/preemption limits and long-running loops.
...
- [ ] 21. Add parameter coverage for: numeric loop, native sleep/downcall, thread fleet.
- [ ] 22. Execute or wire the required variants: cooperative loop, long loop with sparse polls, thread in native call, many idle threads, allocation pressure trigger.
...
- [ ] 40. Run `openspec validate plab-205-safepoints-ttsp --strict` and repository quality gates.

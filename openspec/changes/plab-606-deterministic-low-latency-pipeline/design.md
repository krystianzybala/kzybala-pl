...
2. **Visualization** provides a deterministic or data-driven model: full pipeline animation, latency budget waterfall, Java/Rust evidence dashboard, overload recovery timeline.
3. **Java track** builds the baseline and optimized variants with focus on UDP or in-process replay ingest, FFM frame view, sharded thread-per-core processing, SPSC handoff, fixed result encoding and JFR/perf verification.
4. **Rust track** builds equivalent variants with focus on equivalent replay/UDP path, borrowed frame view, owned shard state, bounded queues and fixed result encoding.
...
- naive object/queue pipeline
- optimized Java pipeline
- optimized Rust pipeline
- overload profile
- fault/restart profile

### Datasets and Profiles

- 32B/128B/1KiB fixed events
- uniform keys
- hot-key skew
- steady and burst load
...
- Primary focus: UDP or in-process replay ingest, FFM frame view, sharded thread-per-core processing, SPSC handoff, fixed result encoding and JFR/perf verification.
...
- Primary focus: equivalent replay/UDP path, borrowed frame view, owned shard state, bounded queues and fixed result encoding.
...
1. **Phenomenon under measurement** — Can the techniques from the curriculum combine into a small, reproducible, bounded end-to-end runtime?
2. **Primary hypothesis** — A fixed-layout ingest-to-decision pipeline with single-writer shards, bounded rings, zero-copy decode and explicit overload policy can maintain predictable latency under controlled load.
3. **Controlled variables** — the variant axis (naive object/queue pipeline, optimized Java pipeline, optimized Rust pipeline, overload profile, fault/restart profile) and the
   dataset/profile axis (32B/128B/1KiB fixed events, uniform keys, hot-key skew, steady and burst load) are varied one at a time; toolchain,
...
8. **Benchmark scenarios** — naive object/queue pipeline, optimized Java pipeline, optimized Rust pipeline, overload profile, fault/restart profile × 32B/128B/1KiB fixed events, uniform keys, hot-key skew, steady and burst load, each scenario
   selected per process invocation (never mixed in one run), named and
   recorded in the run's provenance.
9. **Expected canonical metrics** — end-to-end p50/p99/p999, throughput, allocation rate, drops/rejects, CPU/core, cache misses, recovery time; imported through the
   plab-003 canonical schema with units, uncertainty and provenance.
10. **Profiler evidence** — repository-supported subset of: macro harness, HdrHistogram, JFR, async-profiler, perf stat/c2c, Criterion support tools;
...
- concluding anything from a run that exhibits: declaring a language winner from one host
- concluding anything from a run that exhibits: omitting correctness checks
- concluding anything from a run that exhibits: using unbounded queues
- concluding anything from a run that exhibits: hiding warm-up/startup
- concluding anything from a run that exhibits: hand-tuning one implementation only
...
15. **Completion and verification gates** — the tasks.md completion gate
    ...
    accepted by the batch preflight, `openspec validate plab-606-deterministic-low-latency-pipeline --strict`,
    and evidence maturity claimed no higher than derived
    (`docs/evidence-maturity.md`). Learning outcomes: assemble a bounded runtime; defend every latency claim with evidence; explain where Java and Rust differ and where hardware dominates.

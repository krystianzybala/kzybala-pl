...
- [ ] 7. Create the required visual explanations: loop-shape transformations, annotated check-elimination assembly, safety/performance matrix.
- [ ] 8. Add terminology, assumptions and a short pre-lab diagnostic question.
- [ ] 9. Document the known traps: using unsafe as the default answer; out-of-bounds UB in Rust; assuming MemorySegment is automatically slower; benchmarking different validation guarantees.
- [ ] 10. Add accessible textual/table fallbacks for every essential visualization.

## 3. Java Implementation

- [ ] 11. Implement the Java baseline and required variants with focus on C2 range-check elimination, array vs MemorySegment access, loop unswitching and vectorization diagnostics.
...
- [ ] 16. Implement the Rust baseline and required variants with focus on safe indexing, iterators, get_unchecked in isolated unsafe variants and LLVM optimization reports.
...
- [ ] 21. Add parameter coverage for: primitive arrays, slices/subranges, strided access.
- [ ] 22. Execute or wire the required variants: canonical indexed loop, opaque limit provider, irregular index array, safe iterator, isolated unchecked access.
...
- [ ] 40. Run `openspec validate plab-106-bounds-checks-loop-shape --strict` and repository quality gates.

...
- [x] 7. Create the required visual explanations: loop-shape transformations, annotated check-elimination assembly, safety/performance matrix.
- [x] 8. Add terminology, assumptions and a short pre-lab diagnostic question.
- [x] 9. Document the known traps: using unsafe as the default answer; out-of-bounds UB in Rust; assuming MemorySegment is automatically slower; benchmarking different validation guarantees.
- [x] 10. Add accessible textual/table fallbacks for every essential visualization.

## 3. Java Implementation

- [x] 11. Implement the Java baseline and required variants with focus on C2 range-check elimination, array vs MemorySegment access, loop unswitching and vectorization diagnostics.
...
- [x] 16. Implement the Rust baseline and required variants with focus on safe indexing, iterators, get_unchecked in isolated unsafe variants and LLVM optimization reports.
...
- [x] 21. Add parameter coverage for: primitive arrays, slices/subranges, strided access.
- [x] 22. Execute or wire the required variants: canonical indexed loop, opaque limit provider, irregular index array, safe iterator, isolated unchecked access.
...
- [ ] 26. Capture these metrics: ns/element, instructions, branches, assembly checks, vector width.
- [ ] 27. Capture representative evidence using supported tools from: JMH, Criterion, perfasm, -XX diagnostics where supported, cargo asm, LLVM optimization remarks.
...
- [x] 40. Run `openspec validate plab-106-bounds-checks-loop-shape --strict` and repository quality gates.

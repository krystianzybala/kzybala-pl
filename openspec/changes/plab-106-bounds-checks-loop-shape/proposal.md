# Proposal: Bounds Checks and Loop Shape
...
- Include these controlled variants: canonical indexed loop, opaque limit provider, irregular index array, safe iterator, isolated unchecked access.
- Exercise these datasets/profiles: primitive arrays, slices/subranges, strided access.
- Record at least these metrics: ns/element, instructions, branches, assembly checks, vector width.
- Capture evidence using repository-supported subsets of: JMH, Criterion, perfasm, -XX diagnostics where supported, cargo asm, LLVM optimization remarks.

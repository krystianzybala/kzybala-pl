# Design: Bounds Checks and Loop Shape
...
When can compilers eliminate bounds checks, and when does a harmless-looking loop shape prevent it?

## Hypothesis

Canonical loops with stable limits enable range-check elimination, while aliasing, opaque calls and irregular indexing may retain checks or inhibit vectorization.

## Learning Outcomes

- write optimizer-friendly loops
- prove when checks disappear
- keep unsafe code narrow and justified

## Experience Flow

1. **Theory** explains the hardware/runtime mechanism and vocabulary.
2. **Visualization** provides a deterministic or data-driven model: loop-shape transformations, annotated check-elimination assembly, safety/performance matrix.
3. **Java track** builds the baseline and optimized variants with focus on C2 range-check elimination, array vs MemorySegment access, loop unswitching and vectorization diagnostics.
4. **Rust track** builds equivalent variants with focus on safe indexing, iterators, get_unchecked in isolated unsafe variants and LLVM optimization reports.
...
- canonical indexed loop
- opaque limit provider
- irregular index array
- safe iterator
- isolated unchecked access
...
- Criterion
- perfasm
- -XX diagnostics where supported
- cargo asm
...
- Primary focus: C2 range-check elimination, array vs MemorySegment access, loop unswitching and vectorization diagnostics.
...
- Primary focus: safe indexing, iterators, get_unchecked in isolated unsafe variants and LLVM optimization reports.

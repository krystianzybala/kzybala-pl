# Delta for lab-106-bounds-checks-loop-shape

## ADDED Requirements

### Requirement: Performance question and falsifiable hypothesis
The laboratory SHALL state the question “When can compilers eliminate bounds checks, and when does a harmless-looking loop shape prevent it?” and the hypothesis “Canonical loops with stable limits enable range-check elimination, while aliasing, opaque calls and irregular indexing may retain checks or inhibit vectorization.” before presenting results.
...
The laboratory SHALL provide runnable Java implementations focused on C2 range-check elimination, array vs MemorySegment access, loop unswitching and vectorization diagnostics.
...
The laboratory SHALL provide runnable Rust implementations focused on safe indexing, iterators, get_unchecked in isolated unsafe variants and LLVM optimization reports.
...
The laboratory SHALL include the controlled variants: canonical indexed loop, opaque limit provider, irregular index array, safe iterator, isolated unchecked access.
...
The laboratory SHALL exercise these data profiles where applicable: primitive arrays, slices/subranges, strided access.
...
The laboratory SHALL capture and correctly label: ns/element, instructions, branches, assembly checks, vector width.
...
The laboratory SHALL attach relevant evidence from the repository-supported subset of JMH, Criterion, perfasm, -XX diagnostics where supported, cargo asm, LLVM optimization remarks.
...
The laboratory SHALL explicitly discuss these traps: using unsafe as the default answer; out-of-bounds UB in Rust; assuming MemorySegment is automatically slower; benchmarking different validation guarantees.
...
The laboratory SHALL provide data-driven or deterministic visualizations covering: loop-shape transformations, annotated check-elimination assembly, safety/performance matrix.

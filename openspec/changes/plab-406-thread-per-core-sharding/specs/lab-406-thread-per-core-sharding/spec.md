# Delta for lab-406-thread-per-core-sharding

## ADDED Requirements

### Requirement: Performance question and falsifiable hypothesis
The laboratory SHALL state the question “Can ownership partitioning remove synchronization from the hot path?” and the hypothesis “Stable key-to-core sharding and single-writer state can reduce coherence and locks, but routing, skew and rebalancing become first-class costs.” before presenting results.
...
The laboratory SHALL provide runnable Java implementations focused on platform threads, affinity integration boundary, per-thread state, SPSC ingress and deterministic reduction.
...
The laboratory SHALL provide runnable Rust implementations focused on std threads, core affinity boundary, owned shard state and channels/rings.
...
The laboratory SHALL include the controlled variants: shared concurrent map, fixed shards with mutex, single-writer thread-per-core, skewed key distribution, rebalance simulation.
...
The laboratory SHALL exercise these data profiles where applicable: account/order state updates, per-key counters, partitioned aggregation.
...
The laboratory SHALL capture and correctly label: end-to-end p50/p99/p999, throughput, cross-core transfers, skew, queue depth, rebalance cost.
...
The laboratory SHALL attach relevant evidence from the repository-supported subset of custom load harness, JFR, async-profiler, perf c2c, Criterion/custom Rust harness.
...
The laboratory SHALL explicitly discuss these traps: hiding routing cost; using random keys that eliminate realistic skew; pinning without topology disclosure; calling shared-nothing globally lock-free.
...
The laboratory SHALL provide data-driven or deterministic visualizations covering: ownership map, routing pipeline, per-shard load heatmap.

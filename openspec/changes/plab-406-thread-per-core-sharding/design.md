# Design: Thread-per-Core and Shared-Nothing Sharding
...
2. **Visualization** provides a deterministic or data-driven model: ownership map, routing pipeline, per-shard load heatmap.
3. **Java track** builds the baseline and optimized variants with focus on platform threads, affinity integration boundary, per-thread state, SPSC ingress and deterministic reduction.
4. **Rust track** builds equivalent variants with focus on std threads, core affinity boundary, owned shard state and channels/rings.
...
- Primary focus: platform threads, affinity integration boundary, per-thread state, SPSC ingress and deterministic reduction.
...
- Primary focus: std threads, core affinity boundary, owned shard state and channels/rings.
...
1. **Phenomenon under measurement** — Can ownership partitioning remove synchronization from the hot path?
2. **Primary hypothesis** — Stable key-to-core sharding and single-writer state can reduce coherence and locks, but routing, skew and rebalancing become first-class costs.
3. **Controlled variables** — the variant axis (shared concurrent map, fixed shards with mutex, single-writer thread-per-core, skewed key distribution, rebalance simulation) and the
   dataset/profile axis (account/order state updates, per-key counters, partitioned aggregation) are varied one at a time; toolchain,
...
8. **Benchmark scenarios** — shared concurrent map, fixed shards with mutex, single-writer thread-per-core, skewed key distribution, rebalance simulation × account/order state updates, per-key counters, partitioned aggregation, each scenario
   selected per process invocation (never mixed in one run), named and
   recorded in the run's provenance.
9. **Expected canonical metrics** — end-to-end p50/p99/p999, throughput, cross-core transfers, skew, queue depth, rebalance cost; imported through the
   plab-003 canonical schema with units, uncertainty and provenance.
10. **Profiler evidence** — repository-supported subset of: custom load harness, JFR, async-profiler, perf c2c, Criterion/custom Rust harness;
...
- concluding anything from a run that exhibits: hiding routing cost
- concluding anything from a run that exhibits: using random keys that eliminate realistic skew
- concluding anything from a run that exhibits: pinning without topology disclosure
- concluding anything from a run that exhibits: calling shared-nothing globally lock-free
...
15. **Completion and verification gates** — the tasks.md completion gate
    ...
    accepted by the batch preflight, `openspec validate plab-406-thread-per-core-sharding --strict`,
    and evidence maturity claimed no higher than derived
    (`docs/evidence-maturity.md`). Learning outcomes: design single-writer shards; handle hot-key skew; include routing and recovery costs.

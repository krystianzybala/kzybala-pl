# Delta for lab-103-branch-prediction

## ADDED Requirements

### Requirement: Performance question and falsifiable hypothesis
The laboratory SHALL state the question “When does a simple conditional become more expensive than the work guarded by it?” and the hypothesis “Unpredictable branches produce measurable front-end stalls, while sorted or biased data allows hardware predictors and JIT/compiler transformations to recover throughput.” before presenting results.
...
The laboratory SHALL provide runnable Java implementations focused on Branchy loops, branchless transformations, C2-generated assembly and data-dependent control flow.
...
The laboratory SHALL provide runnable Rust implementations focused on Branchy loops, iterator formulations, branchless masks and generated assembly.
...
The laboratory SHALL include the controlled variants: 90/10 biased branch, 50/50 random branch, sorted input, branchless mask/select.
...
The laboratory SHALL exercise these data profiles where applicable: byte flags, integer thresholds, mixed hot/cold records.
...
The laboratory SHALL capture and correctly label: ns/element, branches, branch-misses, IPC, cycles/element.
...
The laboratory SHALL attach relevant evidence from the repository-supported subset of JMH, Criterion, perf stat, perf record, perfasm, cargo asm.
...
The laboratory SHALL explicitly discuss these traps: regenerating random data inside measurement; compiler removing the branch; confusing vectorization with branch prediction; using one data distribution.
...
The laboratory SHALL provide data-driven or deterministic visualizations covering: branch predictor state diagram, branch-miss vs distribution chart, annotated assembly.

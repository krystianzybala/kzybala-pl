...
1. **Phenomenon under measurement** — When does a simple conditional become more expensive than the work guarded by it?
2. **Primary hypothesis** — Unpredictable branches produce measurable front-end stalls, while sorted or biased data allows hardware predictors and JIT/compiler transformations to recover throughput.
3. **Controlled variables** — the variant axis (90/10 biased branch, 50/50 random branch, sorted input, branchless mask/select) and the
   dataset/profile axis (byte flags, integer thresholds, mixed hot/cold records) are varied one at a time; toolchain,
...
8. **Benchmark scenarios** — 90/10 biased branch, 50/50 random branch, sorted input, branchless mask/select × byte flags, integer thresholds, mixed hot/cold records, each scenario
   selected per process invocation (never mixed in one run), named and
   recorded in the run's provenance.
9. **Expected canonical metrics** — ns/element, branches, branch-misses, IPC, cycles/element; imported through the
   plab-003 canonical schema with units, uncertainty and provenance.
10. **Profiler evidence** — repository-supported subset of: JMH, Criterion, perf stat, perf record, perfasm, cargo asm;
...
- concluding anything from a run that exhibits: regenerating random data inside measurement
- concluding anything from a run that exhibits: compiler removing the branch
- concluding anything from a run that exhibits: confusing vectorization with branch prediction
- concluding anything from a run that exhibits: using one data distribution
...
15. **Completion and verification gates** — the tasks.md completion gate
    ...
    accepted by the batch preflight, `openspec validate plab-103-branch-prediction --strict`,
    and evidence maturity claimed no higher than derived
    (`docs/evidence-maturity.md`). Learning outcomes: connect data distribution to branch misses; distinguish branchless wins from extra-work losses; read relevant perf counters.

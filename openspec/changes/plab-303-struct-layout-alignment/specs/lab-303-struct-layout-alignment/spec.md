# Delta for lab-303-struct-layout-alignment

## ADDED Requirements

### Requirement: Performance question and falsifiable hypothesis
The laboratory SHALL state the question “How do field order, alignment and cache-line boundaries affect footprint and access cost?” and the hypothesis “Reordering fields and separating contended values can reduce padding or coherence traffic, while packed layouts may create unaligned access penalties.” before presenting results.
...
The laboratory SHALL explain the relevant memory & data mechanism before exposing optimized source code.
...
The laboratory SHALL provide runnable Java implementations focused on MemoryLayout.structLayout, paddingLayout, alignment, JOL and VarHandle coordinates.
...
The laboratory SHALL provide runnable Rust implementations focused on repr(C), repr(align), repr(packed) safety constraints and offset inspection.
...
The laboratory SHALL include the controlled variants: natural layout, poor field order, optimized field order, cache-line aligned, packed/unaligned controlled variant.
...
The laboratory SHALL exercise these data profiles where applicable: mixed primitive record, header plus payload, producer/consumer counters.
...
The laboratory SHALL capture and correctly label: bytes/record, ns/access, cache-line crossings, unaligned load evidence.
...
The laboratory SHALL attach relevant evidence from the repository-supported subset of JOL, FFM layouts, JMH, Criterion, offset_of/size_of, perfasm, cargo asm.
...
The laboratory SHALL explicitly discuss these traps: assuming field declaration order is universal; unsafe references to packed fields; padding away footprint without measuring array density; using 64-byte lines as universal without documenting host.
...
The laboratory SHALL provide data-driven or deterministic visualizations covering: offset map, cache-line overlay, footprint vs access chart.

...
- [x] 7. Create the required visual explanations: offset map, cache-line overlay, footprint vs access chart.
- [x] 8. Add terminology, assumptions and a short pre-lab diagnostic question.
- [x] 9. Document the known traps: assuming field declaration order is universal; unsafe references to packed fields; padding away footprint without measuring array density; using 64-byte lines as universal without documenting host.
- [x] 10. Add accessible textual/table fallbacks for every essential visualization.

## 3. Java Implementation

- [x] 11. Implement the Java baseline and required variants with focus on MemoryLayout.structLayout, paddingLayout, alignment, JOL and VarHandle coordinates.
...
- [x] 16. Implement the Rust baseline and required variants with focus on repr(C), repr(align), repr(packed) safety constraints and offset inspection.
...
- [x] 21. Add parameter coverage for: mixed primitive record, header plus payload, producer/consumer counters.
- [x] 22. Execute or wire the required variants: natural layout, poor field order, optimized field order, cache-line aligned, packed/unaligned controlled variant.
...
- [ ] 26. Capture these metrics: bytes/record, ns/access, cache-line crossings, unaligned load evidence.
- [ ] 27. Capture representative evidence using supported tools from: JOL, FFM layouts, JMH, Criterion, offset_of/size_of, perfasm, cargo asm.
...
- [x] 40. Run `openspec validate plab-303-struct-layout-alignment --strict` and repository quality gates.

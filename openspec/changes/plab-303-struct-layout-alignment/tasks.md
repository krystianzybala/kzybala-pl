...
- [ ] 7. Create the required visual explanations: offset map, cache-line overlay, footprint vs access chart.
- [ ] 8. Add terminology, assumptions and a short pre-lab diagnostic question.
- [ ] 9. Document the known traps: assuming field declaration order is universal; unsafe references to packed fields; padding away footprint without measuring array density; using 64-byte lines as universal without documenting host.
- [ ] 10. Add accessible textual/table fallbacks for every essential visualization.

## 3. Java Implementation

- [ ] 11. Implement the Java baseline and required variants with focus on MemoryLayout.structLayout, paddingLayout, alignment, JOL and VarHandle coordinates.
...
- [ ] 16. Implement the Rust baseline and required variants with focus on repr(C), repr(align), repr(packed) safety constraints and offset inspection.
...
- [ ] 21. Add parameter coverage for: mixed primitive record, header plus payload, producer/consumer counters.
- [ ] 22. Execute or wire the required variants: natural layout, poor field order, optimized field order, cache-line aligned, packed/unaligned controlled variant.
...
- [ ] 40. Run `openspec validate plab-303-struct-layout-alignment --strict` and repository quality gates.

...
1. **Phenomenon under measurement** — Why can a saturated system appear healthy when the load generator waits for each response before sending the next request?
2. **Primary hypothesis** — Closed-loop generators under-report latency during stalls; open-loop or omission-corrected measurement exposes queueing and missed schedules.
3. **Controlled variables** — the variant axis (closed loop, open loop fixed rate, Poisson-like arrivals, omission-corrected recording, burst schedule) and the
   dataset/profile axis (periodic 10ms stall, GC/pause injection, bounded server overload) are varied one at a time; toolchain,
...
8. **Benchmark scenarios** — closed loop, open loop fixed rate, Poisson-like arrivals, omission-corrected recording, burst schedule × periodic 10ms stall, GC/pause injection, bounded server overload, each scenario
   selected per process invocation (never mixed in one run), named and
   recorded in the run's provenance.
9. **Expected canonical metrics** — service time, response time, missed schedules, p50/p99/p999, throughput, queue depth; imported through the
   plab-003 canonical schema with units, uncertainty and provenance.
10. **Profiler evidence** — repository-supported subset of: custom load harness, HdrHistogram, JFR, Criterion support code;
...
- concluding anything from a run that exhibits: sleep drift without correction
- concluding anything from a run that exhibits: using offered load different from reported load
- concluding anything from a run that exhibits: dropping missed sends silently
- concluding anything from a run that exhibits: mixing service and response time
...
15. **Completion and verification gates** — the tasks.md completion gate
    ...
    accepted by the batch preflight, `openspec validate plab-605-coordinated-omission-load-generation --strict`,
    and evidence maturity claimed no higher than derived
    (`docs/evidence-maturity.md`). Learning outcomes: detect coordinated omission; build honest open-loop tests; report offered and achieved load separately.

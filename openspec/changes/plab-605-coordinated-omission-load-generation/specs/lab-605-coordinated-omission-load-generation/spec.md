# Delta for lab-605-coordinated-omission-load-generation

## ADDED Requirements

### Requirement: Performance question and falsifiable hypothesis
The laboratory SHALL state the question “Why can a saturated system appear healthy when the load generator waits for each response before sending the next request?” and the hypothesis “Closed-loop generators under-report latency during stalls; open-loop or omission-corrected measurement exposes queueing and missed schedules.” before presenting results.
...
The laboratory SHALL provide runnable Java implementations focused on open/closed-loop load drivers, HdrHistogram correction, scheduled arrivals and backpressure reporting.
...
The laboratory SHALL provide runnable Rust implementations focused on equivalent open/closed-loop generator, monotonic scheduling and histogram correction.
...
The laboratory SHALL include the controlled variants: closed loop, open loop fixed rate, Poisson-like arrivals, omission-corrected recording, burst schedule.
...
The laboratory SHALL exercise these data profiles where applicable: periodic 10ms stall, GC/pause injection, bounded server overload.
...
The laboratory SHALL capture and correctly label: service time, response time, missed schedules, p50/p99/p999, throughput, queue depth.
...
The laboratory SHALL attach relevant evidence from the repository-supported subset of custom load harness, HdrHistogram, JFR, Criterion support code.
...
The laboratory SHALL explicitly discuss these traps: sleep drift without correction; using offered load different from reported load; dropping missed sends silently; mixing service and response time.
...
The laboratory SHALL provide data-driven or deterministic visualizations covering: arrival/response timeline, false vs corrected histogram, offered vs achieved load.

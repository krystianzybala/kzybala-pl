...
1. **Phenomenon under measurement** — What does a nanosecond timestamp actually mean, and how should latency distributions be recorded without distorting the hot path?
2. **Primary hypothesis** — Clock source, timestamp overhead, histogram resolution and sampling strategy materially affect p99 and p999 conclusions.
3. **Controlled variables** — the variant axis (timestamp every operation, sampled timestamps, coordinated omission corrected recording, per-thread histogram merge) and the
   dataset/profile axis (fixed-cost operation, bimodal synthetic latency, periodic pause injection, burst workload) are varied one at a time; toolchain,
...
8. **Benchmark scenarios** — timestamp every operation, sampled timestamps, coordinated omission corrected recording, per-thread histogram merge × fixed-cost operation, bimodal synthetic latency, periodic pause injection, burst workload, each scenario
   selected per process invocation (never mixed in one run), named and
   recorded in the run's provenance.
9. **Expected canonical metrics** — timer overhead, p50, p95, p99, p99.9, max, histogram footprint; imported through the
   plab-003 canonical schema with units, uncertainty and provenance.
10. **Profiler evidence** — repository-supported subset of: JMH, Criterion, HdrHistogram-compatible tooling, perf stat;
...
- concluding anything from a run that exhibits: averages hiding tails
- concluding anything from a run that exhibits: mixing service time and response time
- concluding anything from a run that exhibits: negative or non-monotonic assumptions
- concluding anything from a run that exhibits: recording allocations in the measured path
...
15. **Completion and verification gates** — the tasks.md completion gate
    ...
    accepted by the batch preflight, `openspec validate plab-102-clocks-latency-histograms --strict`,
    and evidence maturity claimed no higher than derived
    (`docs/evidence-maturity.md`). Learning outcomes: interpret latency distributions; separate service time from response time; select histogram precision deliberately.

# Delta for lab-502-memory-mapped-files

## ADDED Requirements

### Requirement: Performance question and falsifiable hypothesis
The laboratory SHALL state the question “When does mmap remove copying, and when do page faults turn latency into a lottery?” and the hypothesis “Mapped access can simplify large-file reads and sharing, but cold pages, dirty writeback and mapping lifecycle dominate tail behavior.” before presenting results.
...
The laboratory SHALL provide runnable Java implementations focused on FileChannel.map or FFM mapping where available, page touching, slicing and cleanup semantics.
...
The laboratory SHALL provide runnable Rust implementations focused on memmap or direct mmap wrapper with safe slice boundaries and flush behavior.
...
The laboratory SHALL include the controlled variants: buffered read, warm mmap sequential, cold mmap sequential, random mapped access, mapped write plus flush.
...
The laboratory SHALL exercise these data profiles where applicable: 64 MiB file, larger-than-RAM optional profile, fixed records.
...
The laboratory SHALL capture and correctly label: ns/record, major/minor faults, read bandwidth, p99 latency, RSS/page cache, flush time.
...
The laboratory SHALL attach relevant evidence from the repository-supported subset of JMH/custom harness, Criterion/custom harness, perf stat, page-fault counters, iostat where available.
...
The laboratory SHALL explicitly discuss these traps: dropping caches without root/disclosure; confusing page cache with process memory; excluding first touch; depending on forced unmapping hacks.
...
The laboratory SHALL provide data-driven or deterministic visualizations covering: virtual-to-page mapping, fault timeline, warm/cold comparison.

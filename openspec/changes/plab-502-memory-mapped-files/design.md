...
1. **Phenomenon under measurement** — When does mmap remove copying, and when do page faults turn latency into a lottery?
2. **Primary hypothesis** — Mapped access can simplify large-file reads and sharing, but cold pages, dirty writeback and mapping lifecycle dominate tail behavior.
3. **Controlled variables** — the variant axis (buffered read, warm mmap sequential, cold mmap sequential, random mapped access, mapped write plus flush) and the
   dataset/profile axis (64 MiB file, larger-than-RAM optional profile, fixed records) are varied one at a time; toolchain,
...
8. **Benchmark scenarios** — buffered read, warm mmap sequential, cold mmap sequential, random mapped access, mapped write plus flush × 64 MiB file, larger-than-RAM optional profile, fixed records, each scenario
   selected per process invocation (never mixed in one run), named and
   recorded in the run's provenance.
9. **Expected canonical metrics** — ns/record, major/minor faults, read bandwidth, p99 latency, RSS/page cache, flush time; imported through the
   plab-003 canonical schema with units, uncertainty and provenance.
10. **Profiler evidence** — repository-supported subset of: JMH/custom harness, Criterion/custom harness, perf stat, page-fault counters, iostat where available;
...
- concluding anything from a run that exhibits: dropping caches without root/disclosure
- concluding anything from a run that exhibits: confusing page cache with process memory
- concluding anything from a run that exhibits: excluding first touch
- concluding anything from a run that exhibits: depending on forced unmapping hacks
...
15. **Completion and verification gates** — the tasks.md completion gate
    ...
    accepted by the batch preflight, `openspec validate plab-502-memory-mapped-files --strict`,
    and evidence maturity claimed no higher than derived
    (`docs/evidence-maturity.md`). Learning outcomes: separate warm and cold mapped performance; account for faults and flush; use mapping lifetimes safely.

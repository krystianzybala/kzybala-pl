...
2. **Visualization** provides a deterministic or data-driven model: process/segment ownership diagram, sequence protocol animation, latency by payload size.
3. **Java track** builds the baseline and optimized variants with focus on FFM/mapped segment, explicit header/layout, VarHandles, sequence protocol and process launcher.
4. **Rust track** builds equivalent variants with focus on mmap-backed region, atomics, repr(C) header and safe wrapper around shared memory.
...
- pipe/socket baseline
- shared memory copy payload
- shared memory slot view
- batched messages
- process restart/recovery scenario

### Datasets and Profiles

- fixed 32-byte/128-byte/1KiB messages
- steady and burst load

### Metrics

- one-way p50/p99/p999
- round-trip latency
- messages/s
- CPU/process
- fault/recovery time
...
- custom process harness
- HdrHistogram
- perf
- JFR
- Criterion support utilities
...
- Primary focus: FFM/mapped segment, explicit header/layout, VarHandles, sequence protocol and process launcher.
...
- Primary focus: mmap-backed region, atomics, repr(C) header and safe wrapper around shared memory.
...
1. **Phenomenon under measurement** — How can two processes exchange fixed messages without sockets or payload copies?
2. **Primary hypothesis** — A mapped SPSC protocol can achieve low one-way latency, but crash recovery, ownership, memory ordering and versioning are harder than the happy-path benchmark.
3. **Controlled variables** — the variant axis (pipe/socket baseline, shared memory copy payload, shared memory slot view, batched messages, process restart/recovery scenario) and the
   dataset/profile axis (fixed 32-byte/128-byte/1KiB messages, steady and burst load) are varied one at a time; toolchain,
...
8. **Benchmark scenarios** — pipe/socket baseline, shared memory copy payload, shared memory slot view, batched messages, process restart/recovery scenario × fixed 32-byte/128-byte/1KiB messages, steady and burst load, each scenario
   selected per process invocation (never mixed in one run), named and
   recorded in the run's provenance.
9. **Expected canonical metrics** — one-way p50/p99/p999, round-trip latency, messages/s, CPU/process, fault/recovery time; imported through the
   plab-003 canonical schema with units, uncertainty and provenance.
10. **Profiler evidence** — repository-supported subset of: custom process harness, HdrHistogram, perf, JFR, Criterion support utilities;
...
- concluding anything from a run that exhibits: using same-process threads and calling it IPC
- concluding anything from a run that exhibits: missing cross-process memory-order proof
- concluding anything from a run that exhibits: leaking stale shared segments
- concluding anything from a run that exhibits: ignoring version mismatch and crash state
...
15. **Completion and verification gates** — the tasks.md completion gate
    ...
    accepted by the batch preflight, `openspec validate plab-503-shared-memory-ipc --strict`,
    and evidence maturity claimed no higher than derived
    (`docs/evidence-maturity.md`). Learning outcomes: implement versioned shared-memory protocol; validate cross-process publication; measure recovery and lifecycle.

...
- [ ] 7. Create the required visual explanations: process/segment ownership diagram, sequence protocol animation, latency by payload size.
- [ ] 8. Add terminology, assumptions and a short pre-lab diagnostic question.
- [ ] 9. Document the known traps: using same-process threads and calling it IPC; missing cross-process memory-order proof; leaking stale shared segments; ignoring version mismatch and crash state.
- [ ] 10. Add accessible textual/table fallbacks for every essential visualization.

## 3. Java Implementation

- [ ] 11. Implement the Java baseline and required variants with focus on FFM/mapped segment, explicit header/layout, VarHandles, sequence protocol and process launcher.
...
- [ ] 16. Implement the Rust baseline and required variants with focus on mmap-backed region, atomics, repr(C) header and safe wrapper around shared memory.
...
- [ ] 21. Add parameter coverage for: fixed 32-byte/128-byte/1KiB messages, steady and burst load.
- [ ] 22. Execute or wire the required variants: pipe/socket baseline, shared memory copy payload, shared memory slot view, batched messages, process restart/recovery scenario.
...
- [ ] 26. Capture these metrics: one-way p50/p99/p999, round-trip latency, messages/s, CPU/process, fault/recovery time.
- [ ] 27. Capture representative evidence using supported tools from: custom process harness, HdrHistogram, perf, JFR, Criterion support utilities.
...
- [ ] 40. Run `openspec validate plab-503-shared-memory-ipc --strict` and repository quality gates.

...
- [ ] 7. Create the required visual explanations: happens-before graph, ordering/fence table, jcstress/loom outcome matrix.
- [ ] 8. Add terminology, assumptions and a short pre-lab diagnostic question.
- [ ] 9. Document the known traps: using performance tests as correctness proof; mapping Java and Rust orderings mechanically; benchmarking an intentionally racy variant without isolation; omitting forbidden outcomes.
- [ ] 10. Add accessible textual/table fallbacks for every essential visualization.

## 3. Java Implementation

- [ ] 11. Implement the Java baseline and required variants with focus on Java Memory Model, VarHandle access modes, volatile, fences and jcstress validation.
...
- [ ] 16. Implement the Rust baseline and required variants with focus on Ordering variants, happens-before reasoning, loom model checking and unsafe-cell boundaries.
...
- [ ] 21. Add parameter coverage for: single-slot mailbox, sequence flag plus payload, counter update.
- [ ] 22. Execute or wire the required variants: plain broken publication demonstration, acquire/release publication, volatile/seq-cst publication, CAS loop, fence-based variant.
...
- [ ] 26. Capture these metrics: ops/s, failed CAS, instructions/fences, correctness outcomes, p99.
- [ ] 27. Capture representative evidence using supported tools from: JMH, jcstress, Criterion, loom, perfasm, cargo asm, perf stat.
...
- [ ] 40. Run `openspec validate plab-402-memory-ordering-atomics --strict` and repository quality gates.

# Design: Memory Ordering: VarHandles and Rust Atomics
...
2. **Visualization** provides a deterministic or data-driven model: happens-before graph, ordering/fence table, jcstress/loom outcome matrix.
3. **Java track** builds the baseline and optimized variants with focus on Java Memory Model, VarHandle access modes, volatile, fences and jcstress validation.
4. **Rust track** builds equivalent variants with focus on Ordering variants, happens-before reasoning, loom model checking and unsafe-cell boundaries.
...
- plain broken publication demonstration
- acquire/release publication
- volatile/seq-cst publication
- CAS loop
- fence-based variant
...
- Primary focus: Java Memory Model, VarHandle access modes, volatile, fences and jcstress validation.
...
- Primary focus: Ordering variants, happens-before reasoning, loom model checking and unsafe-cell boundaries.
...
1. **Phenomenon under measurement** — What correctness and performance guarantees do plain, opaque, acquire/release and sequentially consistent operations provide?
2. **Primary hypothesis** — Weaker valid orderings can reduce fences and contention, but incorrect publication creates rare failures that benchmarks alone may not reveal.
3. **Controlled variables** — the variant axis (plain broken publication demonstration, acquire/release publication, volatile/seq-cst publication, CAS loop, fence-based variant) and the
   dataset/profile axis (single-slot mailbox, sequence flag plus payload, counter update) are varied one at a time; toolchain,
...
8. **Benchmark scenarios** — plain broken publication demonstration, acquire/release publication, volatile/seq-cst publication, CAS loop, fence-based variant × single-slot mailbox, sequence flag plus payload, counter update, each scenario
   selected per process invocation (never mixed in one run), named and
   recorded in the run's provenance.
9. **Expected canonical metrics** — ops/s, failed CAS, instructions/fences, correctness outcomes, p99; imported through the
   plab-003 canonical schema with units, uncertainty and provenance.
10. **Profiler evidence** — repository-supported subset of: JMH, jcstress, Criterion, loom, perfasm, cargo asm, perf stat;
...
- concluding anything from a run that exhibits: using performance tests as correctness proof
- concluding anything from a run that exhibits: mapping Java and Rust orderings mechanically
- concluding anything from a run that exhibits: benchmarking an intentionally racy variant without isolation
- concluding anything from a run that exhibits: omitting forbidden outcomes
...
15. **Completion and verification gates** — the tasks.md completion gate
    ...
    accepted by the batch preflight, `openspec validate plab-402-memory-ordering-atomics --strict`,
    and evidence maturity claimed no higher than derived
    (`docs/evidence-maturity.md`). Learning outcomes: choose the weakest correct ordering; prove publication behavior; distinguish language memory models from hardware.

# Design: MPSC Queues and Producer Contention
...
2. **Visualization** provides a deterministic or data-driven model: contention topology, producer scaling curve, queue occupancy timeline.
3. **Java track** builds the baseline and optimized variants with focus on Atomic sequence claims, bounded capacity, producer batching, shard fan-in and JCTools-like design concepts without copying code.
4. **Rust track** builds equivalent variants with focus on fetch_add/CAS sequencing, bounded slots, per-producer SPSC fan-in and ownership invariants.
...
- 2/4/8/physical-core producers
- uniform and skewed producers
- burst traffic

### Metrics

- aggregate throughput
- producer p99 enqueue
- consumer latency
- CAS failures
- cache transfers
- fairness
...
- Primary focus: Atomic sequence claims, bounded capacity, producer batching, shard fan-in and JCTools-like design concepts without copying code.
...
- Primary focus: fetch_add/CAS sequencing, bounded slots, per-producer SPSC fan-in and ownership invariants.
...
1. **Phenomenon under measurement** — What changes when many producers contend for one consumer-owned stream?
2. **Primary hypothesis** — Shared producer sequencing introduces CAS and cache-line contention; batching, shards or per-producer queues can outperform a single MPSC structure.
3. **Controlled variables** — the variant axis (single shared MPSC, batched claims, per-producer SPSC fan-in, mutex queue baseline, unbounded library queue comparison) and the
   dataset/profile axis (2/4/8/physical-core producers, uniform and skewed producers, burst traffic) are varied one at a time; toolchain,
...
8. **Benchmark scenarios** — single shared MPSC, batched claims, per-producer SPSC fan-in, mutex queue baseline, unbounded library queue comparison × 2/4/8/physical-core producers, uniform and skewed producers, burst traffic, each scenario
   selected per process invocation (never mixed in one run), named and
   recorded in the run's provenance.
9. **Expected canonical metrics** — aggregate throughput, producer p99 enqueue, consumer latency, CAS failures, cache transfers, fairness; imported through the
   plab-003 canonical schema with units, uncertainty and provenance.
10. **Profiler evidence** — repository-supported subset of: JMH custom harness, Criterion/custom harness, perf c2c, perf stat, async-profiler locks/cpu;
...
- concluding anything from a run that exhibits: comparing bounded and unbounded semantics
- concluding anything from a run that exhibits: letting consumer fall behind without reporting occupancy
- concluding anything from a run that exhibits: ignoring producer fairness
- concluding anything from a run that exhibits: using allocation-heavy payloads
...
15. **Completion and verification gates** — the tasks.md completion gate
    ...
    accepted by the batch preflight, `openspec validate plab-404-mpsc-contention --strict`,
    and evidence maturity claimed no higher than derived
    (`docs/evidence-maturity.md`). Learning outcomes: identify MPSC contention points; select fan-in architecture; measure fairness and saturation.

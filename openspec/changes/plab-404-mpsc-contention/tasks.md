...
- [ ] 7. Create the required visual explanations: contention topology, producer scaling curve, queue occupancy timeline.
- [ ] 8. Add terminology, assumptions and a short pre-lab diagnostic question.
- [ ] 9. Document the known traps: comparing bounded and unbounded semantics; letting consumer fall behind without reporting occupancy; ignoring producer fairness; using allocation-heavy payloads.
- [ ] 10. Add accessible textual/table fallbacks for every essential visualization.

## 3. Java Implementation

- [ ] 11. Implement the Java baseline and required variants with focus on Atomic sequence claims, bounded capacity, producer batching, shard fan-in and JCTools-like design concepts without copying code.
...
- [ ] 16. Implement the Rust baseline and required variants with focus on fetch_add/CAS sequencing, bounded slots, per-producer SPSC fan-in and ownership invariants.
...
- [ ] 21. Add parameter coverage for: 2/4/8/physical-core producers, uniform and skewed producers, burst traffic.
- [ ] 22. Execute or wire the required variants: single shared MPSC, batched claims, per-producer SPSC fan-in, mutex queue baseline, unbounded library queue comparison.
...
- [ ] 26. Capture these metrics: aggregate throughput, producer p99 enqueue, consumer latency, CAS failures, cache transfers, fairness.
- [ ] 27. Capture representative evidence using supported tools from: JMH custom harness, Criterion/custom harness, perf c2c, perf stat, async-profiler locks/cpu.
...
- [ ] 40. Run `openspec validate plab-404-mpsc-contention --strict` and repository quality gates.

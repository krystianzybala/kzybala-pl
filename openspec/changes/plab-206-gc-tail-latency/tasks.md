...
- [x] 7. Create the required visual explanations: allocation/live-set timeline, collector trade-off radar, pause percentile plot.
- [x] 8. Add terminology, assumptions and a short pre-lab diagnostic question.
- [x] 9. Document the known traps: tiny heaps chosen to manufacture pauses; different semantic workloads; calling Rust allocation-free by default; comparing throughput-only to latency collectors.
- [x] 10. Add accessible textual/table fallbacks for every essential visualization.

## 3. Java Implementation

- [x] 11. Implement the Java baseline and required variants with focus on G1, ZGC and Shenandoah where available, fixed heap sizing, allocation profiles and JFR GC evidence.
...
- [x] 16. Implement the Rust baseline and required variants with focus on Allocator behavior, Vec reuse, Box-heavy variants and explicit reclamation/drop timing.
...
- [x] 21. Add parameter coverage for: object graph churn, message pipeline, retained cache.
- [x] 22. Execute or wire the required variants: low allocation/reuse, steady high allocation, bursty allocation, growing live set, collector matrix.
...
- [ ] 26. Capture these metrics: allocation rate, pause p50/p99/p999, throughput, CPU overhead, RSS, live set.
- [ ] 27. Capture representative evidence using supported tools from: JFR, GC logs, JMH gc profiler, async-profiler alloc, Criterion, heaptrack or equivalent.
...
- [x] 40. Run `openspec validate plab-206-gc-tail-latency --strict` and repository quality gates.

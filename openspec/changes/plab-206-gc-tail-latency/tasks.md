...
- [ ] 7. Create the required visual explanations: allocation/live-set timeline, collector trade-off radar, pause percentile plot.
- [ ] 8. Add terminology, assumptions and a short pre-lab diagnostic question.
- [ ] 9. Document the known traps: tiny heaps chosen to manufacture pauses; different semantic workloads; calling Rust allocation-free by default; comparing throughput-only to latency collectors.
- [ ] 10. Add accessible textual/table fallbacks for every essential visualization.

## 3. Java Implementation

- [ ] 11. Implement the Java baseline and required variants with focus on G1, ZGC and Shenandoah where available, fixed heap sizing, allocation profiles and JFR GC evidence.
...
- [ ] 16. Implement the Rust baseline and required variants with focus on Allocator behavior, Vec reuse, Box-heavy variants and explicit reclamation/drop timing.
...
- [ ] 21. Add parameter coverage for: object graph churn, message pipeline, retained cache.
- [ ] 22. Execute or wire the required variants: low allocation/reuse, steady high allocation, bursty allocation, growing live set, collector matrix.
...
- [ ] 40. Run `openspec validate plab-206-gc-tail-latency --strict` and repository quality gates.

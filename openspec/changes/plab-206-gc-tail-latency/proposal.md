# Proposal: GC Algorithms and Tail Latency
...
- Include these controlled variants: low allocation/reuse, steady high allocation, bursty allocation, growing live set, collector matrix.
- Exercise these datasets/profiles: object graph churn, message pipeline, retained cache.
- Record at least these metrics: allocation rate, pause p50/p99/p999, throughput, CPU overhead, RSS, live set.
- Capture evidence using repository-supported subsets of: JFR, GC logs, JMH gc profiler, async-profiler alloc, Criterion, heaptrack or equivalent.

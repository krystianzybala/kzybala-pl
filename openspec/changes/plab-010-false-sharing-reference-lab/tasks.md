...
- [ ] 7. Create the required visual explanations: cache-line ownership animation, throughput scaling curve, perf c2c evidence panel.
- [ ] 8. Add terminology, assumptions and a short pre-lab diagnostic question.
- [ ] 9. Document the known traps: using thread count without topology; padding local objects that are not adjacent; claiming volatile itself is the cause; using different memory-order guarantees.
- [ ] 10. Add accessible textual/table fallbacks for every essential visualization.

## 3. Java Implementation

- [ ] 11. Implement the Java baseline and required variants with focus on volatile fields, VarHandles, @Contended where explicitly enabled, manual padding and per-thread ownership.
...
- [ ] 16. Implement the Rust baseline and required variants with focus on atomics, repr(align), padded wrappers and per-thread shards.
...
- [ ] 21. Add parameter coverage for: two threads, physical-core sweep, SMT sibling placement, cross-socket placement where available.
- [ ] 22. Execute or wire the required variants: adjacent counters, manually padded counters, runtime-assisted padding, per-thread shard plus reduction.
...
- [ ] 26. Capture these metrics: ops/s, ns/update, cache-to-cache transfers, LLC misses, CPU utilization.
- [ ] 27. Capture representative evidence using supported tools from: JMH group benchmarks, Criterion/custom threaded harness, perf c2c, perf stat, async-profiler.
...
- [ ] 40. Run `openspec validate plab-010-false-sharing-reference-lab --strict` and repository quality gates.

...
- [x] 7. Create the required visual explanations: memory hierarchy diagram, latency staircase, working-set crossover chart.
- [x] 8. Add terminology, assumptions and a short pre-lab diagnostic question.
- [x] 9. Document the known traps: different dataset generation between languages; page faults in measurement; NUMA migration; comparing element counts with different byte footprints.
- [x] 10. Add accessible textual/table fallbacks for every essential visualization.

## 3. Java Implementation

- [x] 11. Implement the Java baseline and required variants with focus on Primitive arrays, pointer-rich objects, sequential/random access and pre-touching.
...
- [x] 16. Implement the Rust baseline and required variants with focus on Vec, boxed nodes, sequential/random indexing and explicit capacity.
...
- [x] 21. Add parameter coverage for: L1-sized, L2-sized, LLC-sized, 2x LLC, large DRAM-resident.
- [x] 22. Execute or wire the required variants: sequential scan, random permutation, pointer chasing, blocked/tiled traversal.
...
- [ ] 26. Capture these metrics: ns/element, L1 misses, LLC misses, cache references, bandwidth, IPC.
- [ ] 27. Capture representative evidence using supported tools from: JMH, Criterion, perf stat, perf c2c where available, async-profiler cache-miss profile.
...
- [x] 40. Run `openspec validate plab-104-cache-locality-working-set --strict` and repository quality gates.

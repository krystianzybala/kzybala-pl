# Design: Logging, Metrics and Profiling Overhead
...
2. **Visualization** provides a deterministic or data-driven model: instrumentation pipeline, cost stack, latency vs sampling chart.
3. **Java track** builds the baseline and optimized variants with focus on logging guards, parameterized logging, counters, JFR events, async-profiler modes and allocation-aware context propagation.
4. **Rust track** builds equivalent variants with focus on tracing/log macros, disabled-level behavior, atomics, sampling and profiler overhead.
...
- no instrumentation
- disabled eager logging
- disabled lazy logging
- synchronous logging
...
- Primary focus: logging guards, parameterized logging, counters, JFR events, async-profiler modes and allocation-aware context propagation.
...
- Primary focus: tracing/log macros, disabled-level behavior, atomics, sampling and profiler overhead.
...
1. **Phenomenon under measurement** — How much latency and allocation does observability add to the path it is supposed to explain?
2. **Primary hypothesis** — Eager formatting, shared locks, labels and stack walking can dominate hot paths; guarded, structured and sampled instrumentation can retain evidence at bounded cost.
3. **Controlled variables** — the variant axis (no instrumentation, disabled eager logging, disabled lazy logging, synchronous logging, async bounded logging, metrics labels, sampled tracing) and the
   dataset/profile axis (single event, error burst, high-cardinality key, stack trace path) are varied one at a time; toolchain,
...
8. **Benchmark scenarios** — no instrumentation, disabled eager logging, disabled lazy logging, synchronous logging, async bounded logging, metrics labels, sampled tracing × single event, error burst, high-cardinality key, stack trace path, each scenario
   selected per process invocation (never mixed in one run), named and
   recorded in the run's provenance.
9. **Expected canonical metrics** — ns/event, B/event, p99/p999, lost log count, CPU, contention; imported through the
   plab-003 canonical schema with units, uncertainty and provenance.
10. **Profiler evidence** — repository-supported subset of: JMH, Criterion, JFR, async-profiler, perf, allocation profiler;
...
- concluding anything from a run that exhibits: benchmarking logger initialization
- concluding anything from a run that exhibits: unbounded async queue
- concluding anything from a run that exhibits: hiding message loss
- concluding anything from a run that exhibits: high-cardinality metrics without memory accounting
...
15. **Completion and verification gates** — the tasks.md completion gate
    ...
    accepted by the batch preflight, `openspec validate plab-604-observability-overhead --strict`,
    and evidence maturity claimed no higher than derived
    (`docs/evidence-maturity.md`). Learning outcomes: budget observability cost; design bounded async logging; select profiling modes with known overhead.

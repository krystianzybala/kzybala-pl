...
- [ ] 7. Create the required visual explanations: instrumentation pipeline, cost stack, latency vs sampling chart.
- [ ] 8. Add terminology, assumptions and a short pre-lab diagnostic question.
- [ ] 9. Document the known traps: benchmarking logger initialization; unbounded async queue; hiding message loss; high-cardinality metrics without memory accounting.
- [ ] 10. Add accessible textual/table fallbacks for every essential visualization.

## 3. Java Implementation

- [ ] 11. Implement the Java baseline and required variants with focus on logging guards, parameterized logging, counters, JFR events, async-profiler modes and allocation-aware context propagation.
...
- [ ] 16. Implement the Rust baseline and required variants with focus on tracing/log macros, disabled-level behavior, atomics, sampling and profiler overhead.
...
- [ ] 21. Add parameter coverage for: single event, error burst, high-cardinality key, stack trace path.
- [ ] 22. Execute or wire the required variants: no instrumentation, disabled eager logging, disabled lazy logging, synchronous logging, async bounded logging, metrics labels, sampled tracing.
...
- [ ] 26. Capture these metrics: ns/event, B/event, p99/p999, lost log count, CPU, contention.
- [ ] 27. Capture representative evidence using supported tools from: JMH, Criterion, JFR, async-profiler, perf, allocation profiler.
...
- [ ] 40. Run `openspec validate plab-604-observability-overhead --strict` and repository quality gates.

# Delta for lab-203-escape-analysis-scalar-replacement

## ADDED Requirements

### Requirement: Performance question and falsifiable hypothesis
The laboratory SHALL state the question “When is an allocation in Java source code not an allocation at runtime?” and the hypothesis “Non-escaping short-lived objects can be scalar-replaced by C2, while identity, opaque calls and uncommon control flow may force materialization.” before presenting results.
...
The laboratory SHALL provide runnable Java implementations focused on Escape analysis, allocation elimination, object identity, synchronization elimination and diagnostics.
...
The laboratory SHALL provide runnable Rust implementations focused on Stack values, moves, borrowing, Option/result aggregates and explicit heap boxing.
...
The laboratory SHALL include the controlled variants: non-escaping aggregate, returned object, stored into field, passed to opaque call, identity hash or synchronization.
...
The laboratory SHALL exercise these data profiles where applicable: small coordinate/value object, result wrapper, temporary parser state.
...
The laboratory SHALL capture and correctly label: B/op, allocations/op, ns/op, GC events, assembly allocation path.
...
The laboratory SHALL attach relevant evidence from the repository-supported subset of JMH gc profiler, JFR, perfasm, Criterion, heap profiler where available.
...
The laboratory SHALL explicitly discuss these traps: assuming every new allocates; using disabled EA as production recommendation; comparing stack Rust value to heap Java object with different semantics; not checking materialization.
...
The laboratory SHALL provide data-driven or deterministic visualizations covering: escape graph, allocation elimination report, latency/allocation crossover.

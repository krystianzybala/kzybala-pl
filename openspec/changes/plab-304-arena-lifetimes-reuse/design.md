...
1. **Phenomenon under measurement** — Can explicit lifetime regions eliminate allocation pressure without becoming a manual-memory disaster?
2. **Primary hypothesis** — Batch-scoped arenas and reusable buffers can make lifetime cost predictable, but pooling may retain memory, increase contention and complicate ownership.
3. **Controlled variables** — the variant axis (allocate per item, batch arena, thread-local reuse, global pool, bounded pool) and the
   dataset/profile axis (message batches, temporary parse trees, scratch buffers) are varied one at a time; toolchain,
...
8. **Benchmark scenarios** — allocate per item, batch arena, thread-local reuse, global pool, bounded pool × message batches, temporary parse trees, scratch buffers, each scenario
   selected per process invocation (never mixed in one run), named and
   recorded in the run's provenance.
9. **Expected canonical metrics** — B/op, allocations/op, reset/close cost, RSS high-water mark, contention, p99; imported through the
   plab-003 canonical schema with units, uncertainty and provenance.
10. **Profiler evidence** — repository-supported subset of: JMH, JFR, async-profiler alloc/lock, Criterion, heap profiler;
...
- concluding anything from a run that exhibits: unbounded pools
- concluding anything from a run that exhibits: cross-thread ownership ambiguity
- concluding anything from a run that exhibits: pooling tiny objects already scalar-replaced
- concluding anything from a run that exhibits: excluding cleanup cost
...
15. **Completion and verification gates** — the tasks.md completion gate
    ...
    accepted by the batch preflight, `openspec validate plab-304-arena-lifetimes-reuse --strict`,
    and evidence maturity claimed no higher than derived
    (`docs/evidence-maturity.md`). Learning outcomes: model lifetimes explicitly; decide when pooling is harmful; build bounded reuse strategies.

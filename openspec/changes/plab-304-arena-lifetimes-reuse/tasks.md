...
- [ ] 7. Create the required visual explanations: lifetime-region diagram, memory high-water timeline, throughput vs pool contention.
- [ ] 8. Add terminology, assumptions and a short pre-lab diagnostic question.
- [ ] 9. Document the known traps: unbounded pools; cross-thread ownership ambiguity; pooling tiny objects already scalar-replaced; excluding cleanup cost.
- [ ] 10. Add accessible textual/table fallbacks for every essential visualization.

## 3. Java Implementation

- [ ] 11. Implement the Java baseline and required variants with focus on confined/shared arenas, auto arenas where appropriate, buffer reuse, thread-local state and lifecycle tests.
...
- [ ] 16. Implement the Rust baseline and required variants with focus on stack/Vec reuse, bump arenas, object pools, ownership transfer and Drop behavior.
...
- [ ] 21. Add parameter coverage for: message batches, temporary parse trees, scratch buffers.
- [ ] 22. Execute or wire the required variants: allocate per item, batch arena, thread-local reuse, global pool, bounded pool.
...
- [ ] 40. Run `openspec validate plab-304-arena-lifetimes-reuse --strict` and repository quality gates.

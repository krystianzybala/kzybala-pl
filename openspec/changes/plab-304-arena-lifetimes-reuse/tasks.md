...
- [x] 7. Create the required visual explanations: lifetime-region diagram, memory high-water timeline, throughput vs pool contention.
- [x] 8. Add terminology, assumptions and a short pre-lab diagnostic question.
- [x] 9. Document the known traps: unbounded pools; cross-thread ownership ambiguity; pooling tiny objects already scalar-replaced; excluding cleanup cost.
- [x] 10. Add accessible textual/table fallbacks for every essential visualization.

## 3. Java Implementation

- [x] 11. Implement the Java baseline and required variants with focus on confined/shared arenas, auto arenas where appropriate, buffer reuse, thread-local state and lifecycle tests.
...
- [x] 16. Implement the Rust baseline and required variants with focus on stack/Vec reuse, bump arenas, object pools, ownership transfer and Drop behavior.
...
- [x] 21. Add parameter coverage for: message batches, temporary parse trees, scratch buffers.
- [x] 22. Execute or wire the required variants: allocate per item, batch arena, thread-local reuse, global pool, bounded pool.
...
- [ ] 26. Capture these metrics: B/op, allocations/op, reset/close cost, RSS high-water mark, contention, p99.
- [ ] 27. Capture representative evidence using supported tools from: JMH, JFR, async-profiler alloc/lock, Criterion, heap profiler.
...
- [x] 40. Run `openspec validate plab-304-arena-lifetimes-reuse --strict` and repository quality gates.

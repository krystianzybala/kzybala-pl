# Delta for lab-304-arena-lifetimes-reuse

## ADDED Requirements

### Requirement: Performance question and falsifiable hypothesis
The laboratory SHALL state the question “Can explicit lifetime regions eliminate allocation pressure without becoming a manual-memory disaster?” and the hypothesis “Batch-scoped arenas and reusable buffers can make lifetime cost predictable, but pooling may retain memory, increase contention and complicate ownership.” before presenting results.
...
The laboratory SHALL provide runnable Java implementations focused on confined/shared arenas, auto arenas where appropriate, buffer reuse, thread-local state and lifecycle tests.
...
The laboratory SHALL provide runnable Rust implementations focused on stack/Vec reuse, bump arenas, object pools, ownership transfer and Drop behavior.
...
The laboratory SHALL include the controlled variants: allocate per item, batch arena, thread-local reuse, global pool, bounded pool.
...
The laboratory SHALL exercise these data profiles where applicable: message batches, temporary parse trees, scratch buffers.
...
The laboratory SHALL capture and correctly label: B/op, allocations/op, reset/close cost, RSS high-water mark, contention, p99.
...
The laboratory SHALL attach relevant evidence from the repository-supported subset of JMH, JFR, async-profiler alloc/lock, Criterion, heap profiler.
...
The laboratory SHALL explicitly discuss these traps: unbounded pools; cross-thread ownership ambiguity; pooling tiny objects already scalar-replaced; excluding cleanup cost.
...
The laboratory SHALL provide data-driven or deterministic visualizations covering: lifetime-region diagram, memory high-water timeline, throughput vs pool contention.

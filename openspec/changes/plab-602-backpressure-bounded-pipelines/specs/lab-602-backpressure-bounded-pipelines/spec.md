# Delta for lab-602-backpressure-bounded-pipelines

## ADDED Requirements

### Requirement: Performance question and falsifiable hypothesis
The laboratory SHALL state the question “How should a low-latency system behave when producers are faster than consumers?” and the hypothesis “Bounded queues make overload visible and controllable; unbounded buffering preserves throughput briefly while destroying latency and memory predictability.” before presenting results.
...
The laboratory SHALL provide runnable Java implementations focused on bounded rings/queues, offer/fail policies, drop/coalesce, upstream signaling and queue metrics.
...
The laboratory SHALL provide runnable Rust implementations focused on bounded channels/rings, try_send-style policies, coalescing and ownership-safe rejection.
...
The laboratory SHALL include the controlled variants: unbounded queue anti-pattern, bounded reject, bounded block, drop oldest/newest, coalesce by key, load shedding.
...
The laboratory SHALL exercise these data profiles where applicable: steady below capacity, short burst, sustained overload, hot-key skew.
...
The laboratory SHALL capture and correctly label: queue depth, sojourn time, end-to-end p99/p999, drop/reject rate, RSS, recovery time.
...
The laboratory SHALL attach relevant evidence from the repository-supported subset of custom load harness, HdrHistogram, JFR, async-profiler, Criterion support harness.
...
The laboratory SHALL explicitly discuss these traps: reporting only successful requests; hiding drops; using infinite producer retries; comparing different overload policies as equivalent.
...
The laboratory SHALL provide data-driven or deterministic visualizations covering: queue/sojourn timeline, overload phase plot, policy outcome matrix.

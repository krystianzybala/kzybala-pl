...
- [x] 2. For an existing lab, inventory routes, content, benchmark variants and raw assets before editing. (Greenfield lab — inventoried `content/labs/spsc-ring-buffer/` and `content/labs/virtual-platform-event-loop/` as structural templates.)
...
- [x] 7. Create the required visual explanations: queue/sojourn timeline, overload phase plot, policy outcome matrix. (Textual/table forms: the six-policy comparison table in theory.md, the illustrative-run comparison table in benchmark.md, and exercises.md Exercise 3's queue-depth-over-time table — no canvas/JS visualization added, per this lab's accessible-fallback-first content contract.)
- [x] 8. Add terminology, assumptions and a short pre-lab diagnostic question.
- [x] 9. Document the known traps: reporting only successful requests; hiding drops; using infinite producer retries; comparing different overload policies as equivalent.
- [x] 10. Add accessible textual/table fallbacks for every essential visualization.

## 3. Java Implementation

- [x] 11. Implement the Java baseline and required variants with focus on bounded rings/queues, offer/fail policies, drop/coalesce, upstream signaling and queue metrics.
...
- [x] 14. Add allocation, JVM-flag and profiler hooks relevant to the lab. (No allocation/JVM-flag hooks beyond the standard JMH harness are needed — this lab's mechanism is queueing-policy bookkeeping cost, not allocation; documented in theory.md Assumptions and scope.)
- [x] 15. Run formatting, static analysis and Java tests before accepting benchmark output.

## 4. Rust Implementation

- [x] 16. Implement the Rust baseline and required variants with focus on bounded channels/rings, try_send-style policies, coalescing and ownership-safe rejection.
...
- [x] 19. Isolate and document unsafe code; add miri/fuzz/model checks where appropriate. (No `unsafe` used anywhere in this crate — `VecDeque`/`HashMap` are sufficient; nothing to isolate or model-check.)
- [x] 20. Run rustfmt, clippy and Rust tests before accepting benchmark output.

## 5. Benchmark Matrix

- [x] 21. Add parameter coverage for: steady below capacity, short burst, sustained overload, hot-key skew.
- [x] 22. Execute or wire the required variants: unbounded queue anti-pattern, bounded reject, bounded block, drop oldest/newest, coalesce by key, load shedding.
...
- [ ] 26. Capture these metrics: queue depth, sojourn time, end-to-end p99/p999, drop/reject rate, RSS, recovery time. (Requires the native-Linux 5810 host run and a genuinely threaded/timed harness — the deterministic correctness simulation intentionally does not measure wall-clock sojourn/latency.)
- [ ] 27. Capture representative evidence using supported tools from: custom load harness, HdrHistogram, JFR, async-profiler, Criterion support harness. (Requires the native-Linux 5810 host run; JFR/async-profiler are capability-unavailable in this repo's toolchain today regardless.)
...
- [x] 40. Run `openspec validate plab-602-backpressure-bounded-pipelines --strict` and repository quality gates.

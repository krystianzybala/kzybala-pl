...
- [ ] 7. Create the required visual explanations: queue/sojourn timeline, overload phase plot, policy outcome matrix.
- [ ] 8. Add terminology, assumptions and a short pre-lab diagnostic question.
- [ ] 9. Document the known traps: reporting only successful requests; hiding drops; using infinite producer retries; comparing different overload policies as equivalent.
- [ ] 10. Add accessible textual/table fallbacks for every essential visualization.

## 3. Java Implementation

- [ ] 11. Implement the Java baseline and required variants with focus on bounded rings/queues, offer/fail policies, drop/coalesce, upstream signaling and queue metrics.
...
- [ ] 16. Implement the Rust baseline and required variants with focus on bounded channels/rings, try_send-style policies, coalescing and ownership-safe rejection.
...
- [ ] 21. Add parameter coverage for: steady below capacity, short burst, sustained overload, hot-key skew.
- [ ] 22. Execute or wire the required variants: unbounded queue anti-pattern, bounded reject, bounded block, drop oldest/newest, coalesce by key, load shedding.
...
- [ ] 26. Capture these metrics: queue depth, sojourn time, end-to-end p99/p999, drop/reject rate, RSS, recovery time.
- [ ] 27. Capture representative evidence using supported tools from: custom load harness, HdrHistogram, JFR, async-profiler, Criterion support harness.
...
- [ ] 40. Run `openspec validate plab-602-backpressure-bounded-pipelines --strict` and repository quality gates.

# Design: Inlining and Call-Site Shape
...
2. **Visualization** provides a deterministic or data-driven model: call-site profile diagram, inlining tree, performance vs code-size chart.
3. **Java track** builds the baseline and optimized variants with focus on C2 inlining decisions, interfaces, sealed hierarchies, method size and hot/cold splitting.
4. **Rust track** builds equivalent variants with focus on Monomorphization, trait objects, enum dispatch and #[inline] trade-offs.
...
- monomorphic interface call
- bimorphic call
- megamorphic call
- manual switch/enum dispatch
- oversized callee
...
- Primary focus: C2 inlining decisions, interfaces, sealed hierarchies, method size and hot/cold splitting.
...
- Primary focus: Monomorphization, trait objects, enum dispatch and #[inline] trade-offs.
...
1. **Phenomenon under measurement** — How do monomorphic, bimorphic and megamorphic call sites change optimization opportunities?
2. **Primary hypothesis** — Inlining unlocks constant propagation and scalar replacement, but call-site diversity and code-size budgets can block it.
3. **Controlled variables** — the variant axis (monomorphic interface call, bimorphic call, megamorphic call, manual switch/enum dispatch, oversized callee) and the
   dataset/profile axis (pricing functions, codec strategies, validation rules) are varied one at a time; toolchain,
...
8. **Benchmark scenarios** — monomorphic interface call, bimorphic call, megamorphic call, manual switch/enum dispatch, oversized callee × pricing functions, codec strategies, validation rules, each scenario
   selected per process invocation (never mixed in one run), named and
   recorded in the run's provenance.
9. **Expected canonical metrics** — ns/call, instructions, code size, inlining decisions, i-cache misses; imported through the
   plab-003 canonical schema with units, uncertainty and provenance.
10. **Profiler evidence** — repository-supported subset of: JMH, Criterion, JITWatch, perfasm, cargo asm, perf stat;
...
- concluding anything from a run that exhibits: manual devirtualization without maintainability discussion
- concluding anything from a run that exhibits: forcing inline everywhere
- concluding anything from a run that exhibits: comparing trait object to Java sealed dispatch as identical
- concluding anything from a run that exhibits: ignoring code-cache/i-cache cost
...
15. **Completion and verification gates** — the tasks.md completion gate
    ...
    accepted by the batch preflight, `openspec validate plab-202-inlining-call-site-shape --strict`,
    and evidence maturity claimed no higher than derived
    (`docs/evidence-maturity.md`). Learning outcomes: identify call-site shapes; connect inlining to downstream optimization; balance code size and specialization.

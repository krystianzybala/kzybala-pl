...
- [x] 7. Create the required visual explanations: call-site profile diagram, inlining tree, performance vs code-size chart.
- [x] 8. Add terminology, assumptions and a short pre-lab diagnostic question.
- [x] 9. Document the known traps: manual devirtualization without maintainability discussion; forcing inline everywhere; comparing trait object to Java sealed dispatch as identical; ignoring code-cache/i-cache cost.
- [x] 10. Add accessible textual/table fallbacks for every essential visualization.

## 3. Java Implementation

- [x] 11. Implement the Java baseline and required variants with focus on C2 inlining decisions, interfaces, sealed hierarchies, method size and hot/cold splitting.
...
- [x] 16. Implement the Rust baseline and required variants with focus on Monomorphization, trait objects, enum dispatch and #[inline] trade-offs.
...
- [x] 21. Add parameter coverage for: pricing functions, codec strategies, validation rules.
- [x] 22. Execute or wire the required variants: monomorphic interface call, bimorphic call, megamorphic call, manual switch/enum dispatch, oversized callee.
...
- [ ] 26. Capture these metrics: ns/call, instructions, code size, inlining decisions, i-cache misses.
- [ ] 27. Capture representative evidence using supported tools from: JMH, Criterion, JITWatch, perfasm, cargo asm, perf stat.
...
- [x] 40. Run `openspec validate plab-202-inlining-call-site-shape --strict` and repository quality gates.

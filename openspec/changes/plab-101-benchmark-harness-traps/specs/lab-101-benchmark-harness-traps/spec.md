# Delta for lab-101-benchmark-harness-traps

## ADDED Requirements

### Requirement: Performance question and falsifiable hypothesis
The laboratory SHALL state the question “How can a benchmark report convincing numbers while measuring almost nothing useful?” and the hypothesis “Dead-code elimination, constant folding, state leakage, insufficient warm-up and incorrect parameterization can dominate the result more than the code under test.” before presenting results.
...
The laboratory SHALL provide runnable Java implementations focused on JMH state scopes, forks, warm-up, Blackhole, @CompilerControl, parameterization and profilers.
...
The laboratory SHALL provide runnable Rust implementations focused on Criterion configuration, black_box, setup placement, sample size, measurement time and cargo bench profiles.
...
The laboratory SHALL include the controlled variants: constant-foldable input vs runtime input, returned result vs consumed result, setup inside timed region vs outside, single fork/process vs isolated forks/processes.
...
The laboratory SHALL exercise these data profiles where applicable: tiny scalar operation, array reduction, parser with reusable input, stateful counter.
...
The laboratory SHALL capture and correctly label: ns/op, ops/s, confidence interval, variance, allocation/op.
...
The laboratory SHALL attach relevant evidence from the repository-supported subset of JMH, Criterion, perf stat, perfasm or JIT assembly, cargo asm or objdump.
...
The laboratory SHALL explicitly discuss these traps: benchmarking debug builds; sharing mutable state between iterations; comparing default JMH and Criterion settings as if identical; publishing only the best run.
...
The laboratory SHALL provide data-driven or deterministic visualizations covering: benchmark lifecycle timeline, warm-up convergence chart, misleading vs corrected result comparison.

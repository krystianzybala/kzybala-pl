...
- [ ] 7. Create the required visual explanations: benchmark lifecycle timeline, warm-up convergence chart, misleading vs corrected result comparison.
- [ ] 8. Add terminology, assumptions and a short pre-lab diagnostic question.
- [ ] 9. Document the known traps: benchmarking debug builds; sharing mutable state between iterations; comparing default JMH and Criterion settings as if identical; publishing only the best run.
- [ ] 10. Add accessible textual/table fallbacks for every essential visualization.

## 3. Java Implementation

- [ ] 11. Implement the Java baseline and required variants with focus on JMH state scopes, forks, warm-up, Blackhole, @CompilerControl, parameterization and profilers.
...
- [ ] 16. Implement the Rust baseline and required variants with focus on Criterion configuration, black_box, setup placement, sample size, measurement time and cargo bench profiles.
...
- [ ] 21. Add parameter coverage for: tiny scalar operation, array reduction, parser with reusable input, stateful counter.
- [ ] 22. Execute or wire the required variants: constant-foldable input vs runtime input, returned result vs consumed result, setup inside timed region vs outside, single fork/process vs isolated forks/processes.
...
- [ ] 40. Run `openspec validate plab-101-benchmark-harness-traps --strict` and repository quality gates.

# Proposal: Locks, Spin Waiting and Parking
...
- Include these controlled variants: uncontended mutex, short contended critical section, long critical section, CAS loop, spin-then-park hybrid.
- Exercise these datasets/profiles: shared counter, small map update, handoff flag.
- Record at least these metrics: throughput, p99 wait, CPU consumption, context switches, CAS retries, fairness.
- Capture evidence using repository-supported subsets of: JMH, Criterion/custom harness, perf sched, perf stat, async-profiler lock.

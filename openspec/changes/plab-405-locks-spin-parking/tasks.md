...
- [ ] 7. Create the required visual explanations: state transition diagram, wait-time distribution, latency vs CPU chart.
- [ ] 8. Add terminology, assumptions and a short pre-lab diagnostic question.
- [ ] 9. Document the known traps: running only uncontended tests; oversubscribing one language differently; busy spinning on shared CI hosts; ignoring priority inversion/fairness.
- [ ] 10. Add accessible textual/table fallbacks for every essential visualization.

## 3. Java Implementation

- [ ] 11. Implement the Java baseline and required variants with focus on synchronized, ReentrantLock, StampedLock where suitable, LockSupport and Thread.onSpinWait.
...
- [ ] 16. Implement the Rust baseline and required variants with focus on Mutex/RwLock, atomics, spin_loop, yield_now and parking primitives.
...
- [ ] 21. Add parameter coverage for: shared counter, small map update, handoff flag.
- [ ] 22. Execute or wire the required variants: uncontended mutex, short contended critical section, long critical section, CAS loop, spin-then-park hybrid.
...
- [ ] 26. Capture these metrics: throughput, p99 wait, CPU consumption, context switches, CAS retries, fairness.
- [ ] 27. Capture representative evidence using supported tools from: JMH, Criterion/custom harness, perf sched, perf stat, async-profiler lock.
...
- [ ] 40. Run `openspec validate plab-405-locks-spin-parking --strict` and repository quality gates.

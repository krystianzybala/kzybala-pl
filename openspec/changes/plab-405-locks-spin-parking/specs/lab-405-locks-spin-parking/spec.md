# Delta for lab-405-locks-spin-parking

## ADDED Requirements

### Requirement: Performance question and falsifiable hypothesis
The laboratory SHALL state the question “When is a lock cheaper than a lock-free algorithm, and when should a waiter spin, yield or park?” and the hypothesis “Critical-section length, contention and oversubscription determine the winner; lock-free is not a magic adjective that exempts code from physics.” before presenting results.
...
The laboratory SHALL provide runnable Java implementations focused on synchronized, ReentrantLock, StampedLock where suitable, LockSupport and Thread.onSpinWait.
...
The laboratory SHALL provide runnable Rust implementations focused on Mutex/RwLock, atomics, spin_loop, yield_now and parking primitives.
...
The laboratory SHALL include the controlled variants: uncontended mutex, short contended critical section, long critical section, CAS loop, spin-then-park hybrid.
...
The laboratory SHALL exercise these data profiles where applicable: shared counter, small map update, handoff flag.
...
The laboratory SHALL capture and correctly label: throughput, p99 wait, CPU consumption, context switches, CAS retries, fairness.
...
The laboratory SHALL attach relevant evidence from the repository-supported subset of JMH, Criterion/custom harness, perf sched, perf stat, async-profiler lock.
...
The laboratory SHALL explicitly discuss these traps: running only uncontended tests; oversubscribing one language differently; busy spinning on shared CI hosts; ignoring priority inversion/fairness.
...
The laboratory SHALL provide data-driven or deterministic visualizations covering: state transition diagram, wait-time distribution, latency vs CPU chart.

# Design: False Sharing Reference Lab
...
2. **Visualization** provides a deterministic or data-driven model: cache-line ownership animation, throughput scaling curve, perf c2c evidence panel.
3. **Java track** builds the baseline and optimized variants with focus on volatile fields, VarHandles, @Contended where explicitly enabled, manual padding and per-thread ownership.
4. **Rust track** builds equivalent variants with focus on atomics, repr(align), padded wrappers and per-thread shards.
...
- adjacent counters
- manually padded counters
- runtime-assisted padding
- per-thread shard plus reduction

### Datasets and Profiles

- two threads
- physical-core sweep
- SMT sibling placement
- cross-socket placement where available
...
- JMH group benchmarks
- Criterion/custom threaded harness
- perf c2c
- perf stat
- async-profiler
...
- Primary focus: volatile fields, VarHandles, @Contended where explicitly enabled, manual padding and per-thread ownership.
...
- Primary focus: atomics, repr(align), padded wrappers and per-thread shards.

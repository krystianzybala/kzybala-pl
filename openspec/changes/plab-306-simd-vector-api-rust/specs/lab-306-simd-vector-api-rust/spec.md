# Delta for lab-306-simd-vector-api-rust

## ADDED Requirements

### Requirement: Performance question and falsifiable hypothesis
The laboratory SHALL state the question “When does explicit SIMD outperform scalar code, and when does memory bandwidth make it irrelevant?” and the hypothesis “Vectorization helps regular compute-heavy loops with sufficient data, but tails, alignment, masks and bandwidth can dominate.” before presenting results.
...
The laboratory SHALL provide runnable Java implementations focused on Vector API species, masks, loop tails, auto-vectorization contrast and supported JDK module configuration.
...
The laboratory SHALL provide runnable Rust implementations focused on portable SIMD or architecture intrinsics behind a safe boundary, auto-vectorization and target-feature handling.
...
The laboratory SHALL include the controlled variants: scalar baseline, auto-vectorized candidate, explicit SIMD, misaligned input, small-tail-heavy input.
...
The laboratory SHALL exercise these data profiles where applicable: sum/min/max, threshold filter, dot product, byte classification.
...
The laboratory SHALL capture and correctly label: ns/element, cycles/element, vector instructions, bandwidth, speedup by size.
...
The laboratory SHALL attach relevant evidence from the repository-supported subset of JMH, Criterion, perf stat, perfasm, cargo asm, compiler optimization reports.
...
The laboratory SHALL explicitly discuss these traps: benchmarking unsupported CPU features; forgetting scalar tail; comparing different vector widths without disclosure; assuming SIMD helps pointer-heavy code.
...
The laboratory SHALL provide data-driven or deterministic visualizations covering: lane-level animation, size crossover chart, assembly instruction annotation.
